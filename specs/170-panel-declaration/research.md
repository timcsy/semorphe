...
# 🔴 2026-09-02 收線：甲乙丙那三選一，答案是【乙】

上面寫「乙：每個宣告 export 一支型別化的 getter」的代價是
「`app-shell` 又要按 id import 五個檔——id 分支換個地方寫」。

**而那句話錯了**：import 那五支的是 **`app.ts`（組裝點）**，不是 `app-shell`。
而第三十九條護欄明寫「組裝點 `app.ts`（可見，**不入棘輪**）」——
**組裝點認得 id 是被允許的，那正是它的工作**。

> **一個「這樣會把問題換個地方」的反駁，要先確認換到的那個地方
> 是不是本來就該放它的地方。**

⟹ 於是 `mount` 進宣告**不需要任何 `as`**，而 T014 不再卡在型別上——
它卡的只是「那 250 行接線要搬」這件事本身。接手步驟寫在 tasks.md 的
「接手入口」那一節。
