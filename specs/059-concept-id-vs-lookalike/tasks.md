...
- [X] T023 [US3] 確認 Phase 4 已完成且核心層零註解語法——**這是 T024 的前置，不可跳過** — FR-021
- [X] T024 [US3] 把 `comment`／`doc_comment`／`block_comment` 的 `layer` 從 `lang-core` 改為 `universal` — FR-020
- [X] T025 [US3] 執行完備性護欄，確認那三個元件的五條路徑**沒有任何一條變成缺或殼** — FR-022
- [X] T026 [US3] 執行中立性護欄，確認 **24 → 24**。**數字若下降，代表 Phase 4 沒搬乾淨**——那三筆是靠標籤消失的，不是靠搬走 — plan「實作順序」

---

## Phase 6：Polish

- [X] T027 執行全套測試，確認全綠 — FR-030
- [X] T028 執行其餘各條護欄，確認皆未上升 — FR-031
- [X] T029 依 quickstart.md 逐條走一次
