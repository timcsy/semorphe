...
**上面對 ② 和 ③ 的預測是錯的**，而錯的方式值得記下來。

② 搬走的是**語法**（`//`、`/** *​/`、`/* *​/`，以及剝除它們的規則）。但那三個
概念的**身分字串**仍然留在核心——`generators.set('comment', …)` 還在，只是它
現在呼叫的是語言套件推進來的語法。中立性護欄數的是身分，所以 ② 之後數字
**一動也不動**（27 → 27）。

數字掉在 ③——因為護欄的規則本來就寫著「universal 概念拔掉 C++ 後依然存在，
不妨礙 P9」。

**這個順序看起來正是「用宣告刷數字」的形狀**：改一個標籤，三筆違規消失。
唯一讓它不是刷數字的，是 **② 已經先做完了**——那三個概念現在真的與語言無關，
`src/core` 產不出也剝不掉任何 C 家族符號，由「核心層零註解語法」那支測試釘住。

**所以順序約束比計畫寫的時候更硬，不是更軟。** 原本以為 ③ 只是收尾；實際上
③ 才是數字下降的那一步，而它的正當性**完全**由 ② 提供。反過來做的話，那就是
一次貨真價實的刷數字，而且事後從 commit 上看不出差別。
