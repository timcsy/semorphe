...
- [ ] T001 記錄基線：執行 `npm test`、`npx tsc --noEmit`、`npm run test:e2e`，
      把三個數字寫進本檔末尾的「基線紀錄」區塊
- [ ] T002 [P] 確認 41 條護欄的當前基線：執行
...
- [ ] T003 在 `e2e/diagnostics.spec.ts` 補一支斷言：同一則診斷在積木面板與程式碼面板的
      文字**不相等**。⚠️ 入口條件錨在「兩個面板各有 ≥1 則診斷」（合成量），
      **不可錨在「不同」**
- [ ] T004 執行 `npm run test:e2e`，確認 T003 **紅**，
      且紅的原因是「兩個字串相等」而**不是**「找不到診斷」
- [ ] T005 [P] 新建 `tests/integration/audit-diagnostic-labels.test.ts`（第四十二條護欄，硬性零）：
      入口條件「掃到的規則身分數 > 0」；硬性零「規則身分 × 面板 × 語言 的缺漏數 = 0」；
      正向注入（少一份 → 報出來**並指名**哪條規則／哪個面板／哪種語言）；
      反向注入（完整 → 不得亂報）
- [ ] T006 執行 `npx vitest run tests/integration/audit-diagnostic-labels.test.ts`，
      確認 **紅**，且入口條件那支是**綠**的
- [ ] T007 git commit：`test(118): 兩條先紅的防線——面板訊息必須不同、文案必須完備`
...
- [ ] T008 [US1][US2] 改 `src/core/diagnostics.ts`：`Diagnostic` **刪除 `message`**，
      新增 `rule: string` 與 `params: Record<string, string | number>`；
      `DiagnosticRule.message` 改名為 `rule`
- [ ] T009 [US1][US2] 改 `src/core/diagnostics.ts` 的 `runDiagnostics`：
      三處 `push` 帶上參數——`hasInput` 帶 `{ inputName }`；
      `varDeclareNames` 帶 `{ index: i }`（含 `NAME` 單數那一路，`index: 0`）。
      ⚠️ **`switch` 的判定邏輯一行不改**（contracts §四）
- [ ] T010 [US1][US2] 改 `src/core/view-host.ts` 的 `DiagnosticsEvent` 內嵌型別
- [ ] T011 [US1][US2] 改 `src/languages/cpp/diagnostics.ts`：4 條規則的欄位改名；
      🔴 **把 `DIAG_MISSING_VALUE` 拆成兩個身分**——`cpp_print` → `MISSING_VALUE`、
      `cpp_var_declare` → `MISSING_VAR_NAME`（data-model §實體二）
- [ ] T012 執行 `npx tsc --noEmit`——⚠️ **預期整片紅**，那是設計。
      把紅的清單抄進本檔末尾，確認每一處都在計畫的異動範圍內
      （有範圍外的 → 停下來，不要順手改）
- [ ] T013 改 `tests/unit/core/diagnostics.test.ts`：斷言改成看 `rule` 與 `params`；
      🔴 **新增一支**：`int , , ;` 產出三則，而三則的 `params.index` 互不相同
- [ ] T014 git commit：`refactor(118): Diagnostic 帶規則身分與參數，不再帶組好的訊息`
...
- [ ] T015 [P][US1] `src/i18n/zh-TW/blocks.json`：4 key → 6 份
      （3 個規則身分 × 2 個面板），措辭依 data-model §實體三 的方向
- [ ] T016 [P][US2] `src/i18n/en/blocks.json`：同上 6 份
- [ ] T017 [US1] 改 `src/ui/panels/blockly-panel.ts` 的 `onDiagnostics`：
      用 `DIAG_<RULE>_BLOCK` 組裝；🔴 **同一顆積木的多則要合併成一段文字**
      （今天 `setWarningText` 後蓋前，三個空變數只顯示一個）；
      **移除 `|| d.message` 的靜默降級**
- [ ] T018 [US2] 改 `src/ui/panels/monaco-panel.ts` 的 `onDiagnostics`：
      用 `DIAG_<RULE>_CODE` 組裝；**移除 `?? key` 的靜默降級**
- [ ] T019 [US2] 🔴 更正 `src/ui/panels/monaco-panel.ts:222` 的過期註解（FR-007）——
      它今天宣稱訊息該是「學生程度 × 面板」，而「學生程度」已被否決
      （`knowledge/experience.md` 第 99 條）。**第二條軸只有面板**
- [ ] T020 [P] 新建 `tests/unit/ui/diagnostic-message.test.ts`：
      對 3 個規則身分逐一斷言**兩個面板組出的字串不相等**（3 條全數，不抽驗）
- [ ] T021 執行 `npx tsc --noEmit` ＋ `npm test`——T005 的護欄應轉綠
- [ ] T022 執行 `npm run test:e2e`——T003 的斷言應轉綠
- [ ] T023 git commit：`feat(118): 積木側與程式碼側各自組裝診斷訊息`
...
- [ ] T024 [US3] 反向驗證①：暫時拿掉任一份文案 → 護欄必須**紅**，
      且訊息**指名**是哪一條規則／哪個面板／哪種語言。確認後改回
- [ ] T025 [US3] 反向驗證②：暫時把兩個面板的組裝改成同一個 →
      T003 的 e2e 與 T020 的單元測試必須**紅**。確認後改回
- [ ] T026 git commit：`test(118): 反向驗證兩條防線都真的會紅`
...
- [ ] T027 瀏覽器實測（`knowledge/skills/diagnose-in-browser`），三個情境：
      ① `if` 條件空著——兩側訊息不同
      ② 🔴 `int , , ;`——**三個問題都被提到**（今天只顯示一個）
      ③ 切成英文——兩側都變英文，而且仍然不同
- [ ] T028 執行 `npx vitest run tests/integration/audit-*.test.ts`：
      🔴 **41 條護欄的基線數字一個都不該動**。有動 → **停下來查**，不要順手改基線
- [ ] T029 全套：`npx tsc --noEmit` ＋ `npm test` ＋ `npm run test:e2e`，
      對照 T001 的基線，確認**一支都沒少**
- [ ] T030 更新 `knowledge/vision.md` 階段 6.6 驗收④ 標記為完成，
      並記下 research §二 發現的既有缺陷（`int , , ;` 三則不可區分）已一併修掉
- [ ] T031 git commit：`docs(118): 階段 6.6 驗收④ 完成`
...
```
npm test          ______ 支通過（預期起點 4109）
npx tsc --noEmit  ______
npm run test:e2e  ______ 支（預期起點 8）
