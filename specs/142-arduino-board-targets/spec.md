...
### ~~User Story 3 - 三塊板子的腳位不一樣~~ 🔄 **Phase 0 移出本刀**

學生在 Uno 上看到類比腳位是 `A0`–`A5`；換到 ESP32，看到的是它自己的 GPIO 編號。

> 🔄 **移出的理由**（[research.md](research.md) R3）：`properties.values` 是一個
> **靜態宣告**，要它隨目標變需要一個**今天不存在的機制**（屬性的候選值由目標提供），
> 而那會動到 175 顆元件的宣告契約、動態下拉、存檔完整性護欄。
> 憲法第一條：「**新增的複雜度 MUST 有明確的當前需求作為理由**」——
> 而 US1／US2 不需要那個機制。
> ⚠️ **這不是砍掉它，是排序**：它應該成為下一刀，而那一刀的名字是
> 「屬性的候選值由目標提供」——一個比「板子」更一般的機制。
...
- ~~**FR-005**：目標 MUST 能提供不同的腳位常數集合。~~
  🔄 **Phase 0 移出本刀**，見 [research.md](research.md) R3 與下方「明確排除」。
- **FR-006**：既有的 `cpp-beginner`／`c-beginner`／`cpp-competitive` 三個目標的
  可見集合 MUST 完全不變。
- **FR-007**：一顆沒有宣告任何能力需求的概念，MUST 在所有板子上都看得到（預設為「有」）。
- **FR-008**（🔄 **Phase 0 修訂**，見 [research.md](research.md) R2）：系統 MUST 有一條檢查，
  確保**每一個被宣告需要的能力，至少有一個目標提供它**。硬性零。
  ⚠️ 它 MUST 在能力被宣告**之前**就蓋好（憲法 II ＋ `build-guardrail` 6.5）。

  > 🔄 **原文寫的是「可拿性護欄要放寬判準」，而那個前提是錯的**：實測顯示
  > 它算「拿得到」是看**分類定義**，與目標無關（`audit-toolbox-reachability.test.ts:58`）
  > ——原封不動就是綠的。它今天量的其實已經是「在至少一個目標下拿得到」，
  > 只是當時只有一個目標所以看不出差別。
  >
  > **一條護欄的判準有時不必改——要改的是我們對它在量什麼的理解。**
  >
  > 🔴 而這一刀開了一個**新的**洞（宣告了一個沒有任何目標提供的能力 → 那顆元件
  > 在所有板子上都拿不到，而可拿性全綠），FR-008 改成堵那個洞。
...
- 🔄 **腳位常數隨板子變**（原 US3／FR-005）——`properties.values` 是靜態宣告，
  需要一個不存在的機制。**下一刀**：「屬性的候選值由目標提供」。見 R3。
...
| 日期 | 改了什麼 | 為什麼 |
|---|---|---|
| 2026-08-19 Phase 0 | FR-008 從「放寬可拿性判準」→「新增能力供給完備性檢查」 | 原前提錯了，護欄與目標無關（R2） |
| 2026-08-19 Phase 0 | FR-005／US3 移出本刀 | 需要一個不存在的機制而沒有當前需求（R3） |
