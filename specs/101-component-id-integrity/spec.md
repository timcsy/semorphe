...
- **FR-006**: `cpp_priority_queue_declare` MUST **補齊成真元件**（人已拍板 2026-08-07：補齊，不移除）。
  五路齊備 ＋ 概念定義 ＋ 積木投影；歸屬 `<queue>` 模組（`priority_queue` 就在該標頭裡），
  工具箱位置由 E 項的段落導出**自動**落在「堆疊與佇列」——不必編輯任何清單
...
- **SC-004**: `priority_queue<int> pq;` 走完**五路**：辨識得出身分、產得回去、積木渲染得出、抽取得回、執行結果與 `g++ -std=c++17` 一致
- **SC-005**: 全套綠，其餘護欄無一上升
...
| 補齊 `priority_queue` 引入一顆新元件的五路缺口 | 新元件最容易只做三路 | SC-004 逐路驗，**執行那一路與 `g++` 對答案**（B 項就是靠這個抓到 static／參照兩個缺陷） |
