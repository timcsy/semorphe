...
```
code   = project(tree, language, codeStyle)              // Generator
blocks = project(tree, language, locale, blockStyle)     // Renderer
...
> **程式是一棵語義樹，程式碼和積木都是它的參數化投影——程式碼有編碼風格，積木有排版風格，兩者正交且不改語義。概念形成可分層、可映射的代數結構。系統透過開放擴充成長，透過漸進揭露適應不同使用者。積木是認知鷹架——降低外在負荷、在近側發展區內引導學習，並最終退場。**
