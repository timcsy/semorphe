...
describe('DependencyResolver contract', () => {
  ...
  it('should return single edge for known concept', () => {
    const resolver = new MockDependencyResolver()
    resolver.addMapping('lang:print', '<iostream>', '#include <iostream>')
    const edges = resolver.resolve(['lang:print'])
    ...
    expect(edges[0].reason).toBe('lang:print')
  })

  it('should deduplicate edges with same header', () => {
    const resolver = new MockDependencyResolver()
    resolver.addMapping('lang:print', '<iostream>', '#include <iostream>')
    resolver.addMapping('lang:endl', '<iostream>', '#include <iostream>')
    const edges = resolver.resolve(['lang:print', 'lang:endl'])
    expect(edges).toHaveLength(1)
    expect(edges[0].header).toBe('<iostream>')
  })
  ...
  it('should sort edges by header alphabetically', () => {
    ...
    const edges = resolver.resolve(['lang:print', 'vec', 'sort'])
  ...
  it('should ignore unknown concepts', () => {
    const resolver = new MockDependencyResolver()
    resolver.addMapping('lang:print', '<iostream>', '#include <iostream>')
    const edges = resolver.resolve(['lang:print', 'unknown_concept', 'lang:if', 'lang:var_declare'])
    expect(edges).toHaveLength(1)
    expect(edges[0].header).toBe('<iostream>')
  })
  ...
  it('should set reason to the first concept that triggered the dependency', () => {
    const resolver = new MockDependencyResolver()
    resolver.addMapping('lang:print', '<iostream>', '#include <iostream>')
    resolver.addMapping('lang:input', '<iostream>', '#include <iostream>')
    const edges = resolver.resolve(['lang:print', 'lang:input'])
    expect(edges[0].reason).toBe('lang:print')
  })
})
