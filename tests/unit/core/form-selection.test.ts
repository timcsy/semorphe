...
describe('C-1 選擇是全函數——任何輸入都回傳一個 blockType', () => {
  it('★ 軸值取得到 → 回對應的形態', () => {
    ...
    expect(selectForm(containerForm, node, {}).blockType).toBe('synth_stack_push')
  })
  ...
  it('★ 軸值取不到 → 回 fallback，且**不出聲**', () => {
    ...
    const r = selectForm(containerForm, node, {})
  ...
  it('★ 沒有軸 → 直接回唯一的形態', () => {
    ...
    expect(selectForm(singleForm, node, {}).blockType).toBe('synth_plain')
  })

  it('★ 依呈現位置的軸：statement 與 expression 各自選到不同形態', () => {
    const node = createNode('synth_increment', { var: 'i' }, {})
    expect(selectForm(positionForm, node, { position: 'statement' }).blockType).toBe('synth_increment')
    expect(selectForm(positionForm, node, { position: 'expression' }).blockType).toBe('synth_increment_expr')
  })

  it('★ 位置軸而呼叫端沒給位置 → fallback，不得丟例外', () => {
    const node = createNode('synth_increment', { var: 'i' }, {})
    expect(() => selectForm(positionForm, node, {})).not.toThrow()
    expect(selectForm(positionForm, node, {}).blockType).toBe('synth_increment')
  })
})
...
describe('C-5 選擇只讀 node 與呈現位置', () => {
  it('★ 同輸入同輸出（無隱藏狀態）', () => {
    const node = createNode('synth_container_push', { obj: 'st', container_kind: 'stack' }, {})
    const a = selectForm(containerForm, node, {})
    const b = selectForm(containerForm, node, {})
  ...
  it('★ 節點沒有父節點／樹的引用也選得出來', () => {
    ...
    // 因為這個節點是孤立的。
    const orphanNodes = createNode('synth_container_push', { obj: 'q', container_kind: 'queue' }, {})
    expect(selectForm(containerForm, orphanNodes, {}).blockType).toBe('synth_queue_push')
  })
})

describe('FS-1..FS-4 形態集合的不變式', () => {
  it('★ FS-1：forms 非空', () => {
    const bad = { ...singleForm, forms: {} }
    expect(validateFormSet(bad).ok).toBe(false)
  })

  it('★ FS-2：fallback 必須在 forms 的值域裡', () => {
    const bad = { ...containerForm, fallback: 'synth_not_a_form' }
  ...
  it('★ 合法的形態集合必須通過', () => {
    ...
    expect(validateFormSet(containerForm).ok).toBe(true)
    expect(validateFormSet(positionForm).ok).toBe(true)
    expect(validateFormSet(singleForm).ok).toBe(true)
  })
})
...
describe('C-3 同一身分的任兩形態必須等價', () => {
  it('★ 所有形態的 blockType 互不相同', () => {
    ...
    const bad = { ...containerForm, forms: { _: 'synth_n', stack: 'synth_same', queue: 'synth_same' }, fallback: 'synth_n' }
    expect(validateFormSet(bad).ok).toBe(false)
  })
})
...
describe('C-4 反向唯一——一個 blockType 只能屬於一個 conceptId', () => {
  ...
  it('★ 反向：不撞的兩個形態集合必須通過', () => {
    expect(validateFormSet(positionForm, [containerForm]).ok).toBe(true)
  })
})
