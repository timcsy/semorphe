...
describe('版面宣告的三個純函數', () => {
  ...
  it('gridTemplateAreas：四份宣告各自的 CSS 字串', () => {
    ...
    expect(gridTemplateAreas(P('grid'))).toBe('"element space" "relation state"')
  })
  ...
  it('🔴 十字是四格，每格 1×1——「沒有任何一層是特別的」是可量的', () => {
    ...
    expect(c.map((x) => x.layer)).toEqual(['element', 'space', 'relation', 'state'])
    expect(c.every((x) => x.rowSpan === 1 && x.colSpan === 1), '有一格比別人大').toBe(true)
  })

  it('🔴 從「對照」切到「十字」，程式碼與積木的【格子位置】不變（SC-003 的宣告側）', () => {
    ...
    expect(at('grid', 'space')).toMatchObject({ row: 1, col: 2 })
  })
