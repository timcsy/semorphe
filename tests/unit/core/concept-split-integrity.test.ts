...
describe('Concept/BlockDef split integrity', () => {
  ...
  it('should have correct core concept and block counts', () => {
    ...
    // 同日：67 → 59（第三批 8 顆）→ 56（第四批 3 顆）→ 55（第六批）→ 50（第九批）→ 48（第十批）→ 47（第十四批）→ 45（第十五批）→ 44（第十七批）→ 42（第十八、十九批的 array_declare／forward_decl）。
    expect(coreConcepts.length).toBe(43)
    ...
    expect(coreBlocks.length).toBe(53)
  })
