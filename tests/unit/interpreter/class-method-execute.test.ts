...
const counter = (): SemanticNode =>
  n('cpp:class_def', { name: 'Counter' }, {
    public: [
      n('lang:var_declare', { name: 'n', type: 'int' }),
      n('lang:func_def', { name: 'bump', return_type: 'void' }, {
        params: [],
        body: [assign('n', n('lang:arithmetic', { operator: '+' }, { left: [ref('n')], right: [num(1)] }))],
      }),
      n('lang:func_def', { name: 'bumpTwice', return_type: 'void' }, {
        params: [],
        body: [
      ...
      n('lang:func_def', { name: 'get', return_type: 'int' }, {
        params: [],
        body: [n('lang:return', {}, { value: [ref('n')] })],
      }),
    ],
    private: [],
  })
...
describe('類別與方法', () => {
  it('★ 宣告一個類別 → 它的欄位可以實例化', async () => {
    const out = await run(
      prog(counter(), n('lang:var_declare', { name: 'c', type: 'Counter' }),
  ...
  it('★ 兩個實例的方法互不影響', async () => {
    const out = await run(
      prog(counter(),
        n('lang:var_declare', { name: 'a', type: 'Counter' }),
        n('lang:var_declare', { name: 'b', type: 'Counter' }),
  ...
  it('★ 呼叫不存在的方法要出聲', async () => {
    let 訊息 = ''
    try {
      await run(prog(counter(), n('lang:var_declare', { name: 'c', type: 'Counter' }),
  ...
  it('★ 方法裡的區域變數不得洩漏成欄位', async () => {
    const withLocal = n('cpp:class_def', { name: 'L' }, {
      public: [
        n('lang:var_declare', { name: 'f', type: 'int' }),
        n('lang:func_def', { name: 'run', return_type: 'void' }, {
          params: [],
          body: [n('lang:var_declare', { name: '區域', type: 'int' }, { initializer: [num(5)] })],
        }),
      ],
      private: [],
    })
    let 訊息 = ''
    try {
      await run(prog(withLocal, n('lang:var_declare', { name: 'o', type: 'L' }),
...
describe('建構式', () => {
  ...
  const withCtor = (): SemanticNode =>
    n('cpp:class_def', { name: 'P' }, {
      public: [
        n('lang:var_declare', { name: 'v', type: 'int' }),
        n('cpp:constructor', { class_name: 'P' }, {
          params: [n('lang:var_declare', { name: 'a', type: 'int' })],
          body: [assign('v', ref('a'))],
        }),
      ],
      private: [],
  ...
  it('★ 有參數的建構式會被呼叫', async () => {
    const out = await run(
      prog(withCtor(),
        n('lang:var_declare', { name: 'p', type: 'P' }, { initializer: [n('lang:func_call', { name: 'P' }, { args: [num(42)] })] }),
  ...
  it('★ 沒有呼叫建構式時，欄位仍是預設值（不得炸）', async () => {
    const out = await run(
      prog(withCtor(), n('lang:var_declare', { name: 'p', type: 'P' }),
        show(n('cpp:struct_member_access', { obj: 'p', member: 'v' }))),
    )
