...
const str = (v: string): SemanticNode => n('string_literal', { value: v })
...
const make = (varName: string, tag: number, type = 'C'): SemanticNode[] => [
  n('var_declare', { name: varName, type }),
  n('var_assign', { name: varName, member: 'tag' }, { value: [num(tag)] }),
]
...
describe('解構式在離開作用域時執行', () => {
  ...
  it('★ 函式回傳時跑', async () => {
    const out = await run(
      prog(
        withDtor(),
        n('func_def', { name: 'f', return_type: 'void' }, { params: [], body: [...make('x', 5)] }),
        n('func_call', { name: 'f' }, { args: [] }),
        show(str('回來了')),
      ),
    )
  ...
  it('★ 沒有解構式的類別不得出錯', async () => {
    ...
    const out = await run(
      prog(noDtor, n('if_else', {}, { condition: [num(1)], then: [n('var_declare', { name: 'd', type: 'D' })], else: [] }), show(str('OK'))),
    )
  ...
  it('★ 非物件的變數不得觸發任何收尾', async () => {
    ...
    const out = await run(
      prog(
        ...
        n('if_else', {}, {
          condition: [num(1)],
          then: [n('var_declare', { name: 'i', type: 'int' }, { initializer: [num(3)] })],
          else: [],
        }),
        show(str('OK')),
      ),
