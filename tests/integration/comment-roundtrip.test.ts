...
describe('Comment Round-trip (US7)', () => {
  it('should lift line comment to comment node', () => {
    ...
    const commentNode = body.find(n => n.componentId === 'cpp:comment')
  ...
  it('should handle multiple comments', () => {
    ...
    expect(tree).not.toBeNull()
    const body = tree!.children.body
    const comments = body.filter(n => n.componentId === 'cpp:comment')
...
describe('Block Comment Round-trip', () => {
  it('should lift single-line block comment to block_comment node', () => {
    ...
    expect(tree).not.toBeNull()
    const body = tree!.children.body
    const commentNode = body.find(n => n.componentId === 'cpp:block_comment')
    expect(commentNode).toBeDefined()
...
describe('Doc Comment (JSDoc/Doxygen) Round-trip', () => {
  it('should lift /** brief */ to doc_comment with brief', () => {
    ...
    expect(tree).not.toBeNull()
    const body = tree!.children.body
    const docNode = body.find(n => n.componentId === 'cpp:doc_comment')
...
int add(int a, int b) { return a + b; }`
    const tree = liftCode(code)
    const docNode = tree!.children.body.find(n => n.componentId === 'cpp:doc_comment')
    expect(docNode).toBeDefined()
