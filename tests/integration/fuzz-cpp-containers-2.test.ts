...
describe.runIf(hasReferenceCompiler())('模糊測試的回歸：容器第二輪', () => {
  ...
  /**
   * 🟠 **括號形式的建構引數，對「沒有大小可言」的容器整個掉了**
   *（2026-09-18，管線 188 的整合關卡抓到）。
   *
   * ```cpp
   * pair<int,int> p(1, 2);   g++ 的 p.first 是 1 ／ 我們是 0
   * pair<int,int> q{1, 2};   🟢 這一種是好的
   * ```
   *
   * 根因與同一個檔案裡那段「第四種不會報錯」是**同一族的第五種**：
   * 組裝路徑認得 `values`／`source`／`size`＋`fill` 三種槽，而
   * `pair` 的括號兩個引數被當成「大小與填充」——它沒有宣告 `size`，
   * 於是兩條都不成立，**初始值整個從語義樹上消失**。
   *
   * > **一條「我認得的槽有三種」的組裝路徑，第四種不會報錯
   * > ——它會安靜地少一塊，而少掉的那一塊是使用者打的字。**
   *
   * 🟠 **為什麼不現在修**：修法是「把括號的 N 個引數包成聚合初始值」，
   *    而那會讓產碼從 `p(1, 2)` 變成 `p = {1, 2}`——**形式變了**，
   *    要重新走一次那一族的四個面向（第一百一十五／一百一十七／一百一十八條）。
   * 🔴 何時該修：**聚合宣告的括號形式**那一刀（與 `make_tuple` 同一族，
   *    它們都是「用幾個值造一個聚合」）。
   */
  it.fails('[UNSUPPORTED:聚合宣告的括號形式] 🟠 `pair<int,int> p(1, 2)` 的建構引數', async () => {
    await same(`pair<int,int> p(1, 2); cout << p.first << p.second;`, '')
  }, 60_000)
