...
describe('Declarations', () => {
  describe('Single variable', () => {
    ...
    it('lifts int x = 5; correctly', () => {
      ...
      expect(inits[0].conceptId).toBe('cpp:literal_number')
    ...
    it('lifts string name = "hello"; correctly', () => {
      ...
      const inits = node!.children.initializer ?? []
      expect(inits.length).toBe(1)
      expect(inits[0].conceptId).toBe('cpp:literal_string')
  ...
  describe('Variable assignment', () => {
    it('lifts x = 10; correctly', () => {
      ...
      expect(node).not.toBeNull()
      expect(node!.conceptId).toBe('cpp:var_assign')
  ...
  describe('Array declaration', () => {
    it('lifts int arr[10]; correctly', () => {
      ...
      expect(node).not.toBeNull()
      expect(node!.conceptId).toBe('cpp:array_declare')
    })
...
describe('Expressions', () => {
  ...
  describe('Literals', () => {
    it('lifts number literal', () => {
      ...
      expect(n!.conceptId).toBe('cpp:literal_number')
    ...
    it('lifts char literal (strips quotes)', () => {
      ...
      const node = body[0]
      const inits = node?.children.initializer ?? []
      expect(inits[0]?.conceptId).toBe('cpp:literal_char')
  ...
  describe('Arithmetic', () => {
    it.each(['+', '-', '*', '/', '%'])('lifts binary %s', (op) => {
      ...
      expect(n).not.toBeNull()
      expect(n!.conceptId).toBe('cpp:arithmetic')
      expect(n!.properties.operator).toBe(op)
      expect(n!.children.left?.[0]?.conceptId).toBe('cpp:var_ref')
      expect(n!.children.right?.[0]?.conceptId).toBe('cpp:var_ref')
    })
  ...
  describe('Comparison', () => {
    it.each(['>', '<', '>=', '<=', '==', '!='])('lifts %s', (op) => {
      ...
      expect(n).not.toBeNull()
      expect(n!.conceptId).toBe('cpp:compare')
      expect(n!.properties.operator).toBe(op)
    })
  })
  ...
  describe('Logical', () => {
    it.each(['&&', '||'])('lifts %s', (op) => {
      const n = liftExpr(`a ${op} b`)
      expect(n).not.toBeNull()
      expect(n!.conceptId).toBe('cpp:logic')
      expect(n!.properties.operator).toBe(op)
    })
    ...
    it('lifts !x (logic_not)', () => {
      ...
      expect(n).not.toBeNull()
      expect(n!.conceptId).toBe('cpp:logic_not')
    })
  ...
  describe('Unary negate', () => {
    it('lifts -x (negate)', () => {
      const n = liftExpr('-x')
      expect(n).not.toBeNull()
      expect(n!.conceptId).toBe('cpp:negate')
    })
  ...
  describe('Variable reference', () => {
    it('lifts identifier as var_ref', () => {
      ...
      expect(n).not.toBeNull()
      expect(n!.conceptId).toBe('cpp:var_ref')
    ...
    it('lifts true as builtin_constant', () => {
      ...
      expect(n).not.toBeNull()
      expect(n!.conceptId).toBe('cpp:builtin_constant')
...
describe('I/O', () => {
  describe('cout (print)', () => {
    it('lifts cout << "hello"; as print with string value', () => {
      ...
      expect(printNode!.conceptId).toBe('cpp:print')
      const values = printNode!.children.values ?? []
      expect(values.length).toBe(1)
      expect(values[0].conceptId).toBe('cpp:literal_string')
    ...
    it('lifts cout << x << y; as print with two values', () => {
      ...
      expect(values[0].conceptId).toBe('cpp:var_ref')
      expect(values[1].conceptId).toBe('cpp:var_ref')
    })
    ...
    it('lifts cout << "hi" << endl; with endl concept', () => {
      ...
      expect(values.length).toBe(2)
      expect(values[0].conceptId).toBe('cpp:literal_string')
      expect(values[1].conceptId).toBe('cpp:endl')
    })
  ...
  describe('cin (input)', () => {
    it('lifts cin >> x; as input with one variable', () => {
      ...
      expect(inputNode!.conceptId).toBe('cpp:input')
    ...
    it('lifts if (cin >> a >> n >> m) as if with input condition', () => {
      ...
      expect(ifNode!.conceptId).toBe('cpp:if')
      const cond = (ifNode!.children.condition ?? [])[0]
      expect(cond).toBeDefined()
      expect(cond!.conceptId).toBe('cpp:input')
  ...
  describe('cpp:endl', () => {
    it('lifts endl identifier as endl concept (not var_ref)', () => {
      ...
      expect(endlNode.conceptId).toBe('cpp:endl')
    })
...
describe('Control Flow', () => {
  ...
  describe('while loop', () => {
    it('lifts while loop', () => {
      ...
      expect(whileNode!.conceptId).toBe('cpp:loop_while')
    })
  ...
  describe('for loop (counting)', () => {
    it('lifts counting for as count_loop', () => {
      ...
      expect(forNode!.conceptId).toBe('cpp:loop_count')
  ...
  describe('break / continue', () => {
    it('lifts break', () => {
      ...
      expect(breakNode!.conceptId).toBe('cpp:break')
    })
    ...
    it('lifts continue', () => {
      ...
      expect(contNode!.conceptId).toBe('cpp:continue')
    })
...
describe('Functions', () => {
  describe('Function definition', () => {
    it('lifts int main() {}', () => {
      ...
      expect(node).not.toBeNull()
      expect(node!.conceptId).toBe('cpp:func_def')
    ...
    it('lifts function with params', () => {
      ...
      expect(paramChildren[0].conceptId).toBe('param_decl')
  ...
  describe('Function call (statement)', () => {
    it('lifts func(); as func_call', () => {
      ...
      expect(callNode!.conceptId).toBe('cpp:func_call')
  ...
  describe('Return', () => {
    it('lifts return 0;', () => {
      ...
      expect(retNode!.conceptId).toBe('cpp:return')
      const vals = retNode!.children.value ?? []
      expect(vals.length).toBe(1)
      expect(vals[0].conceptId).toBe('cpp:literal_number')
    })
...
describe('Comments', () => {
  it('lifts // line comment (strips prefix)', () => {
    ...
    expect(node).not.toBeNull()
    expect(node!.conceptId).toBe('cpp:comment')
  ...
  it('lifts /* block comment */ (strips delimiters)', () => {
    ...
    expect(node).not.toBeNull()
    expect(node!.conceptId).toBe('cpp:block_comment')
