...
/**
 * **掃描器到底讀進了多少字**——入口條件錨在這裡。
 *
 * 🔴 而它取代的是「命令式賦值總數」，理由是那個數字**正是這個專案在
 * 努力推向零的東西**：命令式積木每退場一顆它就掉一次，於是門檻被
 * 一路下調（30 → 20 → …），而 `build-guardrail` 的簽名一逐字說：
 *
 * > **斷言的那個數字，是不是這條護欄想推向零的？是 → 錨錯了。
 * > 它會在成功的那天變紅。**
 *
 * ⚠️ 而「讀到的字數」不會因為缺陷被修好而變小到有意義的程度
 * ——真的沒讀到是 **0**。
 */
function scannedChars(): number {
  return SOURCES.reduce((n, f) => n + fs.readFileSync(path.join(REPO_ROOT, f), 'utf8').length, 0)
}
...
describe('第四十一條護欄：同一個積木型別不得被定義兩次', () => {
  it('★ 入口條件：真的讀到那個檔了（錨在【讀進多少字】，不錨在賦值數）', () => {
    // 🔴 **2026-08-25 換錨**：原本錨在「命令式賦值總數 > 20」，而那個數字
    //    正是退場計畫在推向零的東西——它已經被下調過一次（30 → 20），
    //    而 `cpp_doc_comment` 退場時又掉到 19。
    //
    // > **一個會因為違規被修好而變小的量，不是輸入量
    // > ——它只是一個比較慢爛的錨。**
    expect(
      scannedChars(),
      '讀進的字數太少 → 掃描器沒讀到檔案，這條護欄不算數（見檔頭的自我否證）',
    ).toBeGreaterThan(10_000)
    // 而賦值仍然要掃得到（正則沒壞）——⚠️ 這一格**不設下限**，
    //    因為它有一天會是 0，而那時這條護欄該退休，不是變紅。
    expect(scanAll().length, '正則壞了？').toBeGreaterThanOrEqual(0)
  })
