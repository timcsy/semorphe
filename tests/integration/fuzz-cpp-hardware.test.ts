...
const componentsIn = (n: SemanticNode, out: string[] = []): string[] => {
  ...
  for (const ks of Object.values(n.slots ?? {})) for (const k of ks) componentsIn(k, out)
  return out
}
...
`
  ...
  /**
   * **帶參數的 `#define`——而這根釘子的理由 2026-09-20 改了**
   *
   * 原文寫著「`history/014` 的墓碑明確否決了模擬 C preprocessor，
   * 而那個判斷**對函式巨集仍然成立**」。🔴 **那句話已經不成立了**：
   * 展開成一個**語句前綴**的函式形巨集（`#define rep(i,n) for(...)`）做了
   *（`languages/cpp/lang/macro-expand.ts`），語料 4 支在用。
   *
   * ⚠️ 而 `PIN_OF(i)` 是**另一支**：它展開成一個**運算式**，嵌在別的算式裡。
   * 那一支要的是**求值**，不是「把一段語句重解一次」——它仍然在墓碑那一側。
   *
   * > **一根釘子如果寫著一個【已經不成立的理由】，
   * > 它會在那個理由被推翻的那天繼續擋著。**
   *
   * 🔴 **何時該修**：運算式位置的函式形巨集那一刀。
   * 觸發條件：**語料或盲測出現第二個獨立來源**（今天語料 0 處，
   * 而 Arduino 那一輪的盲測是第一個）。
   */
  it.todo('[UNSUPPORTED:運算式位置的函式形巨集] 🔴 fuzz_14：帶參數的 `#define` 嵌在算式裡')
