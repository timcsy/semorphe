...
describe('T031: Locale Integration', () => {
  ...
  describe('zh-TW locale completeness', () => {
    ...
    it('should have all dynamic block UI keys', () => {
      ...
      // 🪦 `U_INPUT_LABEL` 於 **2026-09-07** 搬進膠囊（`components/cpp/input/labels/`），
      //    連同 `U_PRINT_MSG`、`C_PRINTF_FORMAT_LABEL`、`C_SCANF_FORMAT_LABEL`。
      //    🔴 **它們是被一個真的漂移逼出來的**：`print` 的畫面文字改成「印出」，
      //    而膠囊裡沒有那個鍵、共用檔裡有一份寫著「輸出」——
      //    於是**改了而畫面沒變，且沒有任何東西報錯**。
      //
      //    > **兩份標籤只要有一份被改，畫面上看到的是【載得比較晚的那一份】
      //    > ——而哪一份載得比較晚，不寫在任何一份裡。**
      //
      //    ⚠️ 從這張清單拿掉一個鍵要附理由：否則「搬走了」與「弄丟了」長得一樣。
      //    （原本那一行的理由留著：🔄 2026-08-26 `cin >>` 的每一格從變數下拉換成接點，
