...
function fetchCorpus(): string[] {
  ...
  for (const f of fs.readdirSync(dir)) {
    ...
    // 🔴 **配對歸配對，長度過濾歸過濾**（2026-08-26）。
    //
    // 這裡本來寫 /`([^`]{4,600})`/g——把**長度條件寫進配對規則**裡。
    // 於是一段少於四個字元的反引號片語（例如註解或測試名字裡的 `&x`）
    // **不匹配**，那個開頭的反引號就沒有被消耗，而它的結尾反引號
    // 變成下一段的**開頭**——從此之後整個檔案的配對全部錯開一格，
    // 把後面真正的語料整段吞掉。
    //
    // 實測：在一支測試的名字裡寫 `&x`，**兩段語料當場從語料庫消失**，
    // 而症狀是「語料總段數少了 2」——第三十二條抓到它，
    // 而它花了不少工夫才被歸因（因為誤差仍然是 0）。
    //
    // > **把一個【內容的條件】寫進【配對的規則】裡，
    // > 不符合條件的那一段不會被丟掉——它會讓後面的每一段都錯位。**
    for (const m of fs.readFileSync(path.join(dir, f), 'utf8').matchAll(/`([^`]*)`/g)) {
      if (m[1].length < 4 || m[1].length > 600) continue
