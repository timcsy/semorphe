...
describe('Four-level lift pipeline', () => {
  ...
  describe('Level 4: Raw code degradation', () => {
     ...
     *
     * ✅ **2026-09-20 修好了，這根釘子拔掉。**
     *
     * 它寫的「為什麼不是現在」逐字是「修法是原文照抄，而那會動到**每一個**
     * 語法錯誤的產出——包含少一個分號這種最常見的，今天它被順手補回去」。
     * 🟢 **而那個顧慮是對的，所以修法不是照抄**：拿七種形狀量下去，
     * 產生器今天分成兩種表現，而**兩種都要留著**：
     *
     * ```
     ...
     * ```
     *
     * 🟢 所以判準換成「**產出不得比使用者寫的少**」（非空白字元的多重集合），
     * 見 `core/projection/code-generator.ts` 的 `honestly`。
     *
     * > **一個「這一段我看不懂」的節點，產出可以比原文【多】（補一個分號），
     * > 不可以比原文【少】——少掉的那幾個字是使用者打的。**
     */
    it('🪦 已拔釘：運算式位置的語法錯誤不得靜默消失', () => {
