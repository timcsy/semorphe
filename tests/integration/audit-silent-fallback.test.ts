 ...
 */
type shape = '型別不符' | '缺子節點' | '值判斷' | '失敗傳遞' | '邊界語義' | '未分類'
...
function classify(condition: string): shape {
  ...
  /**
   * ⚠️ **第六種形狀**（2026-09-20，`s[s.size()]` 那一格落地之後才出現）：
   * 條件是**與一個邊界比對**（`idx === str.length`），而回傳的是
   * **標準規定在那一格的值**——C++11 起 `s[s.size()]` 是一個空字元。
   *
   * > **一個「剛好等於邊界」的分支，與一個「超出範圍就給預設值」的退路
   * > 在語法上都是一個 if——而分得開的地方是【標準有沒有規定那一格】。**
   *
   * 它排在最後：前面五條都更專一，而這一條只認「與長度／大小比對」。
   */
  if (/===\s*\w+(\.\w+)*\.(length|size)\b|\.(length|size)\(?\)?\s*===/.test(condition)) return '邊界語義'
  return '未分類'
}
...
const compatible: Record<shape, decision['decision'][]> = {
  ...
  '失敗傳遞': ['合法'],
  // 邊界語義**只可能是合法**：標準規定了那一格的值，回它不是在防什麼，
  // 也沒有把「我不知道」偽裝成一個答案——判成別的就是矛盾。
  '邊界語義': ['合法'],
  '未分類': ['靜默回退', '合法', '防禦性'],
}
