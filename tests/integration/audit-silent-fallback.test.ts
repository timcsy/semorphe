 ...
 */
type shape = '型別不符' | '缺子節點' | '值判斷' | '失敗傳遞' | '查登錄表' | '未分類'
...
function classify(condition: string): shape {
  /**
   * ⚠️ **第五種形狀**（2026-09-18，接收者重構讓 `m[k]` 要照【值型別】建一格時才出現）。
   *
   * 條件是**問一張登錄表**：「這個名字是不是一個已登錄的容器樣板」。
   * 而兩邊都是「知道答案」——是容器就給一個空容器，不是就給那個型別的預設值。
   *
   * > **一個分岔與一個退路在語法上都是一個 `if`，
   * > 而差別在【另一邊知不知道自己在做什麼】。**
   *
   * 🔴 它必須排在最前面：`componentForContainerTemplate(base)` 不含任何
   *    既有規則的特徵，會一路掉到「未分類」——而那一欄是**要人看**的，
   *    不是一個可以長住的地方。
   * ⚠️ 規則寫得窄（只認「查登錄表」那一族的函式名），拿既有 25 筆命中比對過：**改動 0 筆**。
   */
  if (/^\w*(component|Component)\w*\(|^\w+Template\w*\(/.test(condition)) return '查登錄表'
  ...
  // ⚠️ **第五種形狀**（2026-08-23，C++ 的預設參數字面辨識落地之後才出現）：
  // 條件是**與一個字面值比對**（`t === 'nullptr'`／`t === 'true'`），
  // 而那一族函式是「一行認一種寫法，認不得的最後丟錯」。
  //
  // > **一格辨識與一個退路在語法上都是一個 if，
  // > 而分得開的地方是「認不出來的時候會不會出聲」。**
  //
  // 它排在「缺子節點」**後面**：`x === undefined` 也是字面比對，
  // 而那一筆是真的缺子節點檢查。
  if (/^\w+(\.\w+)*\s*===\s*['"`]/.test(condition)) return '值判斷'
  return '未分類'
}
...
const compatible: Record<shape, decision['decision'][]> = {
  ...
  '失敗傳遞': ['合法'],
  // 🔴 **查登錄表只可能是「合法」**：兩邊都是知道答案的分岔，沒有一邊在防什麼、
  //    也沒有一邊把「不知道」偽裝成一個結果。判成別的等於說它是一個退路。
  '查登錄表': ['合法'],
  '未分類': ['靜默回退', '合法', '防禦性'],
}
