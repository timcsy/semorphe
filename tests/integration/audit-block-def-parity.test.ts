...
describe('spec 163 · 宣告與命令式，逐項比對', () => {
  ...
  it('🎯 報表：命令式定義的積木，宣告式建得出【一樣的形狀】嗎', async () => {
    ...
    // 🔴 **這個錨換過兩次，而前一次沒有救它**（2026-08-26）。
    //
    //    第一版錨在 `> 20`，2026-08-24 下調成 `> 5`，而當時的註解自己寫著
    //    「它原本錨在『今天有幾顆』——於是它會**在清乾淨的那天說謊**」。
    //    ⚠️ **那句話說對了病，而處方只是把同一個錨往下挪。**
    //    2026-08-26 命令式定義掉到 5，它當場變紅。
    //
    // > **一個承認自己會爛的錨，不會因為承認而不爛
    // > ——它只是把成本從「發現」搬到「維護」。**
    //
    //    改錨在**掃描器讀進了多少字**：真的沒讀到是 0，
    //    而它不會因為命令式定義被清掉而變小（那個檔還有一千多行別的東西）。
    expect(regSrc.length, '命令式那個檔沒讀到 → 掃描壞了，不是清乾淨了')
      .toBeGreaterThan(10_000)
    // 🟢 顆數仍然量、**但不設下限**——它有一天會是 0，
    //    而那天這條護欄的這一維該退休，不是變紅。
    // eslint-disable-next-line no-console
    console.log(`  命令式定義 ${imperativeTypes.size} 顆（不設下限：歸零是目標）`)
    ...
    //
    // 🔴 **而上面那段話的處方也是錯的，同一天（2026-08-26）就爛了。**
    //
    //    它寫著「錨改成掃描讀到了多少命令式定義（**合成量，不隨清償變小**）」
    //    ——`imperativeTypes.size` **正是這把刀要推向零的量**，
    //    而那句話宣稱的恰好相反。
    //
    // > **一句斷言「這個錨是合成量」的註解，擋不住它其實是缺陷計數
    // > ——寫下它的人（我）在同一個檔案裡連續三次認錯同一個東西。**
    //
    //    判準（`build-guardrail` 簽名三）只有一句：
    //    **這個量會不會因為違規被修好而變小？** 會 → 錨錯了。
    //
    // 🟢 改錨在**掃描器讀進了多少字**。
    expect(regSrc.length, '命令式那個檔沒讀到 → 掃描壞了').toBeGreaterThan(10_000)
    ...
    // 🔴 **兩個棘輪 2026-08-26 改成硬性零**（十顆命令式定義清完那天）。
    //
    //    ⚠️ 當時我在基線裡寫「這條護欄的母體現在是空的……退不退休是人的決定」
    //    ——**那個判斷弄反了它的角色**。
    //
    ...
    //    修一筆要付多少？       便宜——把形狀寫進那顆元件自己的 forms/blocks.json
    //    ```
    //    ⚠️ 而「便宜」是**量出來的**：今天七顆全部這樣做完，
    //    而其中四顆的命令式那份**是錯的**（參數名字收不下、標籤鍵打錯、
    //    型別下拉是靜態的、兩顆同族的清單不一樣）。
    expect(differ.map((d) => `${d.t}：${d.why}`),
      '🔴 宣告與命令式對同一顆積木說了不同的話，而**使用者看到的是命令式那份**\n'
      + '   （它沒有守衛、後跑、直接覆寫）。\n'
      + '   ⚠️ **要逐筆判哪一邊對**，不准用「把宣告改成跟命令式一樣」刷數字\n'
      + '     ——兩個方向都有實例（`retire-imperative-block` §1）。')
      .toEqual([])
    // 🔴 **第二個硬性零**：兩份定義都有、而宣告是空殼的那些。
    //    ⚠️ 少了它，`differ` 歸零那天報表會說「清乾淨了」——而 2026-08-26 還差 7 顆。
    expect(uncompared,
      '🔴 這幾顆兩份定義都有，而**宣告是空殼**（連 `message0` 都沒有）\n'
      + '   → 比對器把它們略過，**而略過與通過在報表上長得一模一樣**。\n'
      + '   ⚠️ 它與上面那一支**必須一起看**：一顆積木的宣告從「有內容但不一樣」\n'
      + '     退成「空殼」，會讓落差數下降而看起來像進步。')
      .toEqual([])
    void fs; void pathMod
  })
