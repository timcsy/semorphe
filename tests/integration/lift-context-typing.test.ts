...
describe('脈絡有型別時，方法辨識成專屬概念', () => {
  it('★ `s.clear()`（s 是 string）→ 字串專屬', () => {
    expect(
      components('int main(){ string s; s.clear(); }'),
      '字串的 clear 被辨識成通用容器版——專屬身分掉了',
    ).toContain('cpp:string_clear')
  })

  it('★ `s.push_back(c)`（s 是 string）→ 字串專屬', () => {
    expect(components("int main(){ string s; s.push_back('a'); }")).toContain('cpp:string_append_char')
  })
})
...
describe('脈絡沒有型別時，保守地留在通用版', () => {
  ...
  it('★ 型別不明的變數 → 通用版，**不得猜**', () => {
    ...
    const c = components('int main(){ unknownVar.clear(); }')
  ...
  it('★ 同名變數在內層被遮蔽時，用內層的型別', () => {
    ...
    const c = components('int main(){ string s; { vector<int> s; s.clear(); } }')
...
describe('★ 自我驗證：脈絡真的被填了', () => {
  it('沒有這支的話，一個「什麼都不填」的實作也會讓上面的保守測試通過', () => {
    ...
    // 這支把它們的前提釘住：型別追蹤不是空的。
    expect(components('int main(){ string s; s.clear(); }')).toContain('cpp:string_clear')
  })
})
