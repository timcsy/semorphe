...
describe('自我驗證：這條護欄真的量得到東西', () => {
  ...
  it('★ 掃描器有真的掃到東西（第 10 步）', () => {
    ...
    const r = new PatternRenderer()
    r.loadBlockSpecs(specs)
    expect(r.render({ id: 'x', conceptId: 'var_declare', properties: { name: 'a', type: 'int' }, children: {} } as never))
      .not.toBeNull()
  })
  ...
  it('★ 已知答案：`doc_comment` 有顯式 fields，兩邊一致；`block_comment` 靠推導', () => {
    ...
    expect(走一圈('doc_comment', 'brief'), 'doc_comment 有顯式 fields，本來就該保住').toBe('«v»')
  })
})
