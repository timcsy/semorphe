 ...
 *
 * Generated by /component.fuzz cpp hard containers 10 on 2026-03-13.
 * Agent A (isolated) generated 10 programs; Agent B verified roundtrip.
 *
 * Results:
 * - fuzz_6 (vector-only): PASS (P1 stable, vector components correct)
 ...
 * - fuzz_3 (map frequency): PASS (map components now implemented)
 * - fuzz_7 (map operations): PASS (map components now implemented)
 * - map fuzz (declare+erase+count): PASS (P1 stable)
 * - fuzz_2: EXPECTED_DEGRADATION (array initializer list, istringstream)
 * - fuzz_4 (set intersection): PASS (set components now implemented)
 * - fuzz_5 (BFS combo): PASS (all container components now implemented)
 * - fuzz_9 (set dedup): PASS (set components now implemented)
...
describe('C++ Container Fuzz Tests', () => {
  ...
  // PASS — P1 stable, all vector components correctly lifted and generated
...
cout << rows << endl;`
  ...
  // EXPECTED_DEGRADATION — stack components now implemented, but program uses
  ...
  describe('stack fuzz: bracket matching (stack components only)', () => {
...
cout << st.empty() << endl;`
  ...
  // Now PASS — map components implemented in Phase 9 <map> pipeline.
...
cout << freq.empty() << endl;`
    ...
    it('should lift map declare, count, erase components', () => {
      const tree = liftCode(code)
      expect(tree).not.toBeNull()
      const components = collectComponents(tree)
  ...
  // Now PASS — set components implemented in Phase 9 <set> pipeline.
...
cout << result.empty() << endl;`
    ...
    it('should lift set declare, insert, count components', () => {
      const tree = liftCode(code)
      expect(tree).not.toBeNull()
      const components = collectComponents(tree)
  ...
  // Now PASS — all container components implemented.
...
cout << q.empty() << endl;`
    ...
    it('should lift queue, map, set declare components', () => {
      const tree = liftCode(code)
      expect(tree).not.toBeNull()
      const components = collectComponents(tree)
...
cout << endl;`
    ...
    it('should lift queue declare and FIFO operations', () => {
      ...
      const components = collectComponents(tree)
      expect(components.has('cpp:queue_declare')).toBe(true)
      expect(components.has('cpp:container_push')).toBe(true) // shared .push()
      expect(components.has('cpp:queue_front')).toBe(true)
      expect(components.has('cpp:container_pop')).toBe(true) // shared .pop()
    })
...
cout << s.count(5) << endl;`
    ...
    it('should lift set declare, insert, erase, count components', () => {
      const tree = liftCode(code)
      expect(tree).not.toBeNull()
      const components = collectComponents(tree)
      expect(components.has('cpp:set_declare')).toBe(true)
...
cout << mp.empty() << endl;`
    ...
    it('should lift map declare, erase, count components', () => {
      const tree = liftCode(code)
      expect(tree).not.toBeNull()
      const components = collectComponents(tree)
      expect(components.has('cpp:map_declare')).toBe(true)
