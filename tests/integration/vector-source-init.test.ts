...
describe('概念身分與五路', () => {
  it('初始值掛在 source 底下，且不是被丟掉', () => {
    ...
    const decls = collect(tree, (n) => n.conceptId === 'cpp_vector_declare')
  ...
  it('負向：元素列表仍走 values，不得改走 source', () => {
    const decls = collect(liftMain('vector<int> v = {3, 1, 4};'), (n) => n.conceptId === 'cpp_vector_declare')
  ...
  it('負向：`vector<int> v(5)` 是建構子引數，兩個子節點都不得有東西', () => {
    ...
    const decls = collect(liftMain('vector<int> v(5);'), (n) => n.conceptId === 'cpp_vector_declare')
    expect(decls[0].children?.source ?? []).toHaveLength(0)
  ...
  it.fails('（已知缺陷）pair 的初始值仍然掉——**掉得對稱，所以來回轉換是綠的**', () => {
    ...
    const decls = collect(
      ...
      (n) => n.conceptId === 'cpp_pair_declare',
    )
