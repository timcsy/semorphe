...
describe('護欄：雙重真相（JSON blockDef ／ 動態註冊）', () => {
  ...
  it('★ 兩份清單各自都取到了——空清單與零違規產出一樣', () => {
    // 🔴 **這個錨換過**（2026-08-26，第十次撞上同一個形狀）。
    //
    //    原本錨在 `dynamicBodies.size > 10`——而**動態註冊的顆數正是這條護欄
    //    要推向零的那個量**（`build-guardrail` 簽名一）。最後一顆命令式積木
    //    （`cpp_raw_code`）退場那天它剛好掉到 10，於是**一次成功讓入口條件變紅**。
    //
    // > **一個錨在「缺陷還在不在」上的入口條件，會在缺陷清光的那天變紅
    // > ——而那時它看起來像護欄壞了。**
    //
    //    改錨在**掃描器讀進了多少字**：真的沒讀到是 0，而它不會因為
    //    命令式定義被清掉而變小（那個檔還有一千多行別的東西）。
    expect(registrar.length, '動態註冊那個檔沒讀到 → 報表的每個數字都是假的')
      .toBeGreaterThan(10_000)
    expect(json.size, 'JSON 定義那份沒載入 → 同上').toBeGreaterThan(10)
    // 🟢 而顆數仍然量、**但不設下限**——它有一天會是 0，
    //    而那天這條護欄該退休，不是變紅。
    // eslint-disable-next-line no-console
    console.log(`  動態註冊 ${dynamicBodies.size} 顆（不設下限：歸零是目標）`)
  })
  ...
  /**
   * 🔴 **硬性零**（2026-08-26 由棘輪改成）。
   *
   * ## 為什麼改
   *
   * 命令式積木定義歸零那天（`history/163`），這條護欄量的那一側空了
   * ——`動態註冊 0`，於是「兩處都有定義」永遠是 0。
   *
   * 當時我在基線裡寫「⚠️ 一條母體永遠是空的護欄，與一條沒有人跑的護欄
   * 是同一件事……退不退休是人的決定」。**而那個判斷弄反了它的角色。**
   *
   * > **母體空掉的護欄，正是唯一擋得住它再長回來的東西。**
   *
   * `build-guardrail` 那句「母體永遠是空的護欄 ≒ 沒有人跑的護欄」講的是
   * **量既有的債**。而這一條現在不是溫度計，是**閘門**：
   * 明天有人在 `block-registrar.ts` 裡寫一行 `Blockly.Blocks['xx'] = {…}`，
   * **只有它會出聲**。
   *
   * ## 為什麼是硬性零而不是棘輪（`build-guardrail` 6.8 的兩問）
   *
   * ```
   * 留一筆規範還成立嗎？   ❌ 「積木的形狀處處是資料」留一個口就是假的
   * 修一筆要付多少？       便宜——把那顆的形狀寫進它自己的 forms/blocks.json
   *                        （今天七顆全部這樣做完，而且五顆順帶修掉了真的缺陷）
   * 別台機器一樣嗎？       ✅ 純靜態掃描這個檔
   * ```
   */
  it('🔴 硬性零：同一顆積木不得有兩處定義', () => {
    expect(
      both,
      `這幾顆積木有兩處定義：${both.join('、')}\n` +
        '  JSON 是消費者（PatternRenderer）讀的那一份，而**命令式那份沒有守衛、後跑**\n' +
        '  ——於是使用者看到的是它，而宣告躺著。\n' +
        '  🟢 修法：把形狀寫進那顆元件自己的 `forms/blocks.json`\n' +
        '     （`knowledge/skills/retire-imperative-block/`——那把刀 2026-08-26 把\n' +
        '      十顆清完，而其中四顆的命令式那份【是錯的】）。',
    ).toEqual([])
  })

  /**
   * 🔴 **同一件事的另一面**：那個檔裡不得再長出命令式的積木定義。
   *
   * ⚠️ 上面那一支比的是**交集**（兩處都有）。一顆**只有**命令式定義的積木
   * 不在交集裡——於是它從那一支的眼下溜過去，而它是更嚴重的那一種：
   * **連宣告都沒有，唯一真相就住在視圖層。**
   */
  it('🔴 硬性零：`block-registrar.ts` 裡不得有命令式的積木定義', () => {
    const found = [...registrar.matchAll(/Blockly\.Blocks\['([a-z_0-9]+)'\]\s*=\s*\{/g)].map((m) => m[1])
    expect(
      found,
      `這個檔裡又長出命令式的積木定義：${found.join('、')}\n` +
        '  ⚠️ 積木的形狀是**元件的資料**，不是視圖層的程式碼。\n' +
        '  形狀表達不出來時**補建構子的參數**，不要在這裡寫一份實作\n' +
        '  ——判準見 `retire-imperative-block` §2「表達不完要分兩層問」。',
    ).toEqual([])
  })
})
