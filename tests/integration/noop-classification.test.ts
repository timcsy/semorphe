...
const CASES: Case[] = [
  ...
  { id: 'comment', code: `// 一行註解\ncout << 1;`, expect: '1', why: '註解不影響輸出' },
  { id: 'block_comment', code: `/* 區塊 */ cout << 1;`, expect: '1', why: '註解不影響輸出' },
  { id: 'doc_comment', code: `/** doc */ cout << 1;`, expect: '1', why: '註解不影響輸出' },
  { id: 'cpp_include', code: `cout << 1;`, expect: '1', why: 'include 在標頭，不影響執行' },
  { id: 'cpp_include_local', code: `cout << 1;`, expect: '1', why: '同上' },
  { id: 'cpp_using_namespace', code: `cout << 1;`, expect: '1', why: 'using 不影響執行' },
  { id: 'cpp_define', code: `#define N 5\ncout << 1;`, expect: '1', why: '巨集展開是墓碑；本身不執行' },
  { id: 'cpp_stringstream_declare', code: `stringstream ss; cout << 1;`, expect: '1', why: '宣告本身無輸出' },
  { id: 'cpp_ifstream_declare', code: `ifstream f; cout << 1;`, expect: '1', why: '宣告本身無輸出' },
  { id: 'cpp_ofstream_declare', code: `ofstream f; cout << 1;`, expect: '1', why: '宣告本身無輸出' },
  { id: 'cpp_pair_declare', code: `pair<int,int> p; cout << 1;`, expect: '1', why: '宣告本身無輸出' },
  { id: 'cpp_raw_code', code: `cout << 1;`, expect: '1', why: '無法辨識時的兜底，不該有語義' },
  { id: 'cpp_raw_expression', code: `cout << 1;`, expect: '1', why: '同上' },

  // ── 有子槽：子槽裡是使用者的程式碼，跑得出來才算沒丟
  { id: 'cpp_case', code: `int x=2; switch(x){ case 2: cout << 22; break; }`, expect: '22', why: 'case 的 body 要跑' },
  { id: 'cpp_default', code: `int x=9; switch(x){ case 1: cout << 1; break; default: cout << 99; }`, expect: '99', why: 'default 的 body 要跑' },
  { id: 'cpp_ifdef', code: `#define N 1\n#ifdef N\ncout << 7;\n#endif`, expect: '7', why: 'N 已定義 → body 該跑（用未定義來測的話，noop 也會給同樣結果）' },
  { id: 'cpp_ifndef', code: `#ifndef N\ncout << 7;\n#endif`, expect: '7', why: 'N 未定義 → body 該跑' },
  { id: 'cpp_static_cast', code: `double d = 3.9; cout << static_cast<int>(d);`, expect: '3', why: '轉型結果要回傳' },
  { id: 'cpp_const_cast', code: `int a = 5; const int c = a; cout << const_cast<int&>(c);`, expect: '5', why: '轉型結果要回傳' },
  { id: 'cpp_dynamic_cast', code: `double d = 2.7; cout << dynamic_cast<int>(d);`, expect: '2', why: '轉型結果要回傳' },
  { id: 'cpp_reinterpret_cast', code: `double d = 1.9; cout << reinterpret_cast<int>(d);`, expect: '1', why: '轉型結果要回傳' },
  { id: 'cpp_namespace_def', code: `namespace N { int f(){ return 7; } }%%cout << N::f();`, expect: '7', why: 'namespace 內的函式要能呼叫' },
  { id: 'cpp_lambda', code: `auto f = [](int x){ return x*2; }; cout << f(21);`, expect: '42', why: 'lambda 的 body 要跑' },
  { id: 'cpp_struct_declare', code: `struct P { int x; };%%P p; p.x = 8; cout << p.x;`, expect: '8', why: '成員要能存取' },
  { id: 'cpp_class_def', code: `class C { public: int v = 3; };%%C c; cout << c.v;`, expect: '3', why: '成員要能存取' },
  { id: 'cpp_constructor', code: `class C { public: int v; C(){ v = 4; } };%%C c; cout << c.v;`, expect: '4', why: '建構式的 body 要跑' },
  { id: 'cpp_destructor', code: `class C { public: ~C(){ cout << 5; } };%%{ C c; } cout << 1;`, expect: '51', why: '解構式的 body 要跑（空 body 測不到，等於沒測）' },
  { id: 'cpp_virtual_method', code: `class C { public: virtual int f(){ return 6; } };%%C c; cout << c.f();`, expect: '6', why: '方法 body 要跑' },
  { id: 'cpp_override_method', code: `class B { public: virtual int f(){ return 1; } };\nclass D : public B { public: int f() override { return 9; } };%%D d; cout << d.f();`, expect: '9', why: '覆寫的 body 要跑' },
  { id: 'cpp_pure_virtual', code: `class B { public: virtual int f() = 0; };\nclass D : public B { public: int f() override { return 5; } };%%D d; cout << d.f();`, expect: '5', why: '純虛擬本身無 body；子類的要跑' },
  { id: 'cpp_operator_overload', code: `struct V { int n; V operator+(V o){ V r; r.n = n + o.n; return r; } };%%V a; a.n=1; V b; b.n=2; cout << (a+b).n;`, expect: '3', why: '運算子的 body 要跑' },
]
