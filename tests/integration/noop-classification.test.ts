...
const CASES: Case[] = [
  ...
  { id: 'cpp:cast_static', code: `double d = 3.9; cout << static_cast<int>(d);`, expect: '3', why: '轉型結果要回傳' },
  { id: 'cpp:cast_const', code: `int a = 5; const int c = a; cout << const_cast<int&>(c);`, expect: '5', why: '轉型結果要回傳' },
  { id: 'cpp:cast_dynamic', code: `double d = 2.7; cout << dynamic_cast<int>(d);`, expect: '2', why: '轉型結果要回傳' },
  { id: 'cpp:cast_reinterpret', code: `double d = 1.9; cout << reinterpret_cast<int>(d);`, expect: '1', why: '轉型結果要回傳' },
  ...
  { id: 'cpp:method_virtual', code: `class C { public: virtual int f(){ return 6; } };%%C c; cout << c.f();`, expect: '6', why: '方法 body 要跑' },
  { id: 'cpp:method_override', code: `class B { public: virtual int f(){ return 1; } };\nclass D : public B { public: int f() override { return 9; } };%%D d; cout << d.f();`, expect: '9', why: '覆寫的 body 要跑' },
  { id: 'cpp:method_virtual_pure', code: `class B { public: virtual int f() = 0; };\nclass D : public B { public: int f() override { return 5; } };%%D d; cout << d.f();`, expect: '5', why: '純虛擬本身無 body；子類的要跑' },
