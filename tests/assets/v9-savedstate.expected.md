...
## ⚠️ 而**存檔重載之後它跑不動了**——這不是改名造成的

2026-08-11 T031 實測：把這份存檔貼回 localStorage 重整，
積木**全部正確還原**（沒有未知型別、程式碼一字不差），
而按執行得到：

```
RUNTIME_ERR_TYPE_MISMATCH: {"%1":"array"}
```

**根因不在投影，在存檔本身**——這份檔案是在**任何改名之前**錄的，
而它的 `tree` 裡已經是：

```
宣告類身分：['cpp:var_declare']      ← 不是 cpp:vector_declare
code 欄位：  int v;                   ← 不是 vector<int> v;
```

也就是說：**課程降級的結果被寫進了存檔。** 使用者用「初學 C++」課程
（不解鎖 `vector`）存了一份用到 `vector` 的程式，**真實就永久地遺失了**
——重載之後不論換什麼課程都救不回來。

> `knowledge/draft/2026-03-11-已知工程待解問題.md:89` 記著 2026-08-09 的修法：
> 「降級只作用在顯示用的**深拷貝**上，原樹保持真實」。
> **那個修法沒有涵蓋存檔這條路。**

⚠️ **不在 spec 116 的範圍內**（116 管的是積木型別的命名），
但這份樣本因此同時是**那個缺陷的證據**——留著它比修好它更急，
因為它是唯一一份「降級污染存檔」的真實檔案。
