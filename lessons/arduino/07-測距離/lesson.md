...
到編輯器做這一題：〈練習：加一顆蜂鳴器，越近叫得越急〉

### 做一個

用超音波量你桌上某個東西的長度，**再拿尺量一次**，把兩個數字都用序列埠印出來。

🔴 這一題的答案不在程式裡，在**兩個數字差多少**。
⚠️ 量五次，你會發現它們不一樣。感測器的讀值**本來就會跳**——
那要怎麼辦？（取平均？取中位數？丟掉最大最小？）跟旁邊的人討論一下你們各選了哪一種。

到編輯器做這一題：〈做一個：拿它去量一個真的東西〉
