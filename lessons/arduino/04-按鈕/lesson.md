...
會被空氣中的電磁雜訊推來推去，`digitalRead` 讀到的是隨機的。
...
到編輯器做這一題：〈練習：按一下切換（⚠️ 比想像的難）〉

### 做一個

按按鈕的次數記起來，**放開三秒之後**，讓燈閃那麼多下。

🔴 這一題把「按一下切換」那個坑再踩一次：你需要**偵測邊緣**，不是偵測狀態。
⚠️ 而「放開三秒」不能用 `delay`，那三秒裡你還要繼續看按鈕。

到編輯器做這一題：〈做一個：按幾下，閃幾下〉
