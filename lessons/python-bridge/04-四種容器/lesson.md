...
⚠️ **`dict` 和 `set` 都用大括號。** 分辨的方式是裡面有沒有冒號。
...
`list` 和 `tuple` 用位置、`dict` 用**鍵**，而寫法一樣是方括號。
...
到編輯器做這一題：〈練習：印出分數最高的那個人的名字〉

### 做一個

一串有重複的數字 `[3, 1, 4, 1, 5, 9, 2, 6, 5, 3]`。
印出原本幾個、去重之後幾個、以及排好的結果。

🔴 這一題用**兩種容器**：一種負責去重，一種負責保留順序。
⚠️ 想一下為什麼不能只用其中一種。

到編輯器做這一題：〈做一個：去掉重複再排好〉
