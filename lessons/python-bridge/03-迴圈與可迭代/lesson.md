...
這和 C++ 的 `for (string n : names)` 一模一樣，只是不用寫型別、不用 `auto`。
...
到編輯器做這一題：〈練習：印出 1 到 10 之間的偶數，不要用 if〉

### 做一個

一串名字 `["甲", "乙", "丙"]`，印出「第 1 名是 甲」這樣三行。

🔴 **規定要用 `enumerate`**，不准自己開一個計數變數。
做完再用 `range(len(names))` 寫一次，比較兩份哪一份比較不容易寫錯。
