...
⚠️ **預設值不要用 list**（`def f(items=[])`）。那個 list 是所有呼叫共用同一個，
...
寫一個函式，讀進使用者輸入並轉成整數。**打錯字的話要求他重打**，而不是當掉。

提示：`while True:` ＋ `try` ＋ 成功就 `return`。

到編輯器做這一題：〈練習：打錯就要求重打，而不是當掉〉

### 做一個

寫一個 `safe_div(a, b)`：`b` **有預設值 1**，而除以 0 的時候回傳「不能除以零」。
用 `safe_div(10, 4)`、`safe_div(10)`、`safe_div(10, 0)` 各叫一次，把結果印出來。

🔴 `safe_div(10)` 印出來是 `10.0` 不是 `10`，想一下為什麼。

到編輯器做這一題：〈做一個：不會當掉的除法〉
