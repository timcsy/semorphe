...
和上一課的 `while` 版本做同一件事，而少了一行（不用自己 `n = n + 1`），
也**少了一個出錯的地方**。
...
到編輯器做這一題：〈練習：九九乘法表的前三列〉

### 做一個

數出 2 到 100 之間有幾個質數（只能被 1 和自己整除）。

🔴 這一題需要**一個迴圈套在另一個迴圈裡**：外圈走每個數，內圈試除。
做完想一下：內圈真的要試到 `n` 嗎？試到哪裡就夠了？

到編輯器做這一題：〈做一個：100 以內有幾個質數〉
