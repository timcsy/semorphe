...
讀進一段英文，數出每個字母出現幾次，印出來。

⚠️ 裁判餵的是 `aba`。
⚠️ 提示：用 `counts.get(c, 0) + 1`，不用先判斷那個鍵在不在。

到編輯器做這一題：〈練習：數出每個字母出現幾次〉

### 做一個

讀進一行英文，印出**每個單字出現幾次**，依字母順序。

🔴 這一題會撞到這一課那個坑：第一次看到某個字的時候，字典裡還沒有它。
⚠️ 裁判用的是 `the cat the hat the end`。

到編輯器做這一題：〈做一個：每個字出現幾次〉
