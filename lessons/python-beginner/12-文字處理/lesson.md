...
到編輯器做這一題：〈練習：出現最多次的那個字（三課接起來）〉

### 做一個

讀進一行英文，印出**最長的那個單字**和它的長度。

⚠️ 裁判用的是 `a program is a projection`。
🔴 **如果有兩個一樣長呢？** 這一題沒有規定，你自己決定要留第一個還是最後一個——而那個決定會出現在你寫的是 `>` 還是 `>=`。

到編輯器做這一題：〈做一個：最長的那個單字〉
