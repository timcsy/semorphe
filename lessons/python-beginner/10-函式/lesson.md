...
到編輯器做這一題：〈練習：寫一個 is_leap(year)，要 return 不要 print〉

### 做一個

寫兩個函式算最大公因數：一個用迴圈，一個叫自己（遞迴）。兩個答案都印出來。

輾轉相除法只有一句話：`gcd(a, b) = gcd(b, a % b)`，而 `b` 變成 0 的時候答案就是 `a`。
⚠️ 裁判用的是 `48` 和 `18`。哪一個版本比較好讀？跟旁邊的人不一定同答案。

到編輯器做這一題：〈做一個：最大公因數，兩種寫法〉
