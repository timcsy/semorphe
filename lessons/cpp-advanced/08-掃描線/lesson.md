...
直覺做法是「每個時刻數一次」，而時間範圍大的話那是 O(範圍 × n)，太慢。
...
到編輯器做這一題：〈練習：加幾個人進去，確認答案是對的〉

### 做一個

三場會議：`1~5`、`2~6`、`3~4`。印出最多有幾場同時在進行。

🔴 把每一場拆成「開始 +1」和「結束 −1」兩個事件，排序，掃一遍。
⚠️ 而兩兩比較的做法是 **O(n²)**，這一題資料小看不出來，n 上萬就跑不完了。

到編輯器做這一題：〈做一個：最多幾場會議同時在開〉
