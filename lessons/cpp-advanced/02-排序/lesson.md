...
> 🟢 **這一題會告訴你差多少**：先用 O(n²) 寫一版跑跑看，
> 再改成先排序——主控台會說「**這一趟走了 N 步**」，而第二次會說
> 它是上一次的百分之幾。
>
> 🎯 而這一關有一個**目標**：**80 步以內**。
> ⚠️ 那不是一道門——**超過了這一題照樣算你過**，它只是告訴你「還有更省的做法」。
>
> ⚠️ 那個「步」是**直譯器走過一顆積木一次**算一步，
> 不是課本的「基本操作次數」——它比課本細，而**兩版的比值仍然說得出真話**。

到編輯器做這一題：〈練習：最接近的兩個數的差〉

### 做一個

一串數字 `5 1 9 3 7`：印出**中位數**，再印出**最大減最小**。

🔴 兩個答案**排序之後都是一行**——而排序之前，兩個都要寫迴圈。
這就是「很多題目排完序就變簡單」的最小例子。
