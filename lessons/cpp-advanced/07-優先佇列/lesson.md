...
> 🟢 而**積木上分得很清楚**：這一顆寫著「**… 的最大元素**」，
> 上一課那顆寫著「… 的頂端元素」——**同一個 `top()`，兩個名字**。
...
到編輯器做這一題：〈練習：合併果子的最小總成本〉

### 做一個

一串數字 `5 1 9 3 7`，印出**最大的三個加起來**是多少。

🔴 用 `priority_queue`：拿三次就好，**不用排序整串**。
⚠️ 想一下：n 很大而 k 很小的時候，這個做法跟「排序完取前 k 個」差在哪裡？
