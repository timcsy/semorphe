...
> `auto` 是「讓編譯器自己看右邊是什麼型別」——
> 積木盤上那一顆的型別格子選「**自動推斷**」就是它。
...
到編輯器做這一題：〈練習：用 upper_bound - lower_bound 數出 3 幾個〉

### 做一個

有序陣列 `1 3 3 3 5 7`。對 `3`、`4`、`7` 三個詢問，各印出它出現幾次。

🔴 **不准用迴圈數**。`upper_bound` 減 `lower_bound` 就是答案。
⚠️ `4` 不在裡面，而那兩個位置會**指向同一個地方**——所以答案是 0，不是錯誤。

到編輯器做這一題：〈做一個：這個數出現幾次（多筆詢問）〉
