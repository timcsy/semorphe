...
> 積木盤上它們寫著「**… 的前端元素**」與「**… 的頂端元素**」
> ——而放進去、拿出來那兩顆是「**推 … 到 … 的頂端／尾端**」
> 與「**從 … 的頂端移除**」（`stack`）／「**從 … 的前端移除**」（`queue`）。
...
> 問「空了沒」那顆積木寫著「**… 是否為空**」。
> 🔴 **`pop` 之前一定要先問它**——對一個空的 stack 呼叫 `pop()`
> 不是錯誤，是**未定義行為**：它可能看起來好好的，直到換一台機器。

到編輯器做這一題：〈練習：用 stack 判斷括號有沒有配對好〉

### 做一個

7 個人圍成一圈（編號 1 到 7），從 1 開始報數，**每數到第 3 個人就出局**，
一直到剩一個人。印出他的編號。

🔴 用 `queue`：**沒出局的就從前面拿出來、排到後面去**。
這一題不用陣列也不用刪除——那正是佇列適合的形狀。
