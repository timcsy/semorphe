...
到編輯器做這一題：〈練習：把兩個 LL 拿掉，看它印出什麼〉

### 做一個

算出 1 加到 **1000000** 的總和，印出來。

🔴 **先用 `int` 寫一次，記下它印出什麼**，再換成 `long long`。
那個錯的答案是負數——而程式**沒有報任何錯**。這就是這一課第一件事。
⚠️ 另外：用迴圈加一百萬次，和用公式一次算完，跑起來差多少？兩種都試。

到編輯器做這一題：〈做一個：1 加到一百萬〉
