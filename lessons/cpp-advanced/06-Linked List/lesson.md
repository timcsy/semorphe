...
到編輯器做這一題：〈練習：在中間插一個 99——而兩行的順序不能反〉

### 做一個

做一條 `1 → 2 → 3` 的串列，**把它反過來**變成 `3 → 2 → 1`，再走一遍印出來。

🔴 你需要三個指標：**前一個、現在這個、下一個**。
⚠️ 少了「下一個」那一個，你會在改掉 `cur->next` 的那一瞬間**把後面整條弄丟**。
先在紙上畫三個箭頭再動手。
