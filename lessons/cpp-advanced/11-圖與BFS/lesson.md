...
> `!q.empty()` 那一格在積木上是「**… 是否為空**」——前面再加一個「不是」。
...
到編輯器做這一題：〈練習：把 queue 換成 stack，看 dist[3] 變什麼〉

### 做一個

五個點，邊是 `0→1`、`0→2`、`1→3`、`2→3`、`3→4`。
從 0 出發，印出每個點的最短步數（`編號 步數`）。

🔴 點 3 有兩條路走得到（經過 1 或經過 2），而它們**一樣長**。
BFS 的 `if (dist[v] == -1)` 那一行就是在說「**第一次到的那次最短**」。

到編輯器做這一題：〈做一個：每個點離起點幾步〉
