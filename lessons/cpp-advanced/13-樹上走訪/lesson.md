...
一般的圖要 `visited` 陣列，而樹上只要擋住「回頭那一步」。
...
加一個 `size` 陣列，算出每個節點的子樹大小。

⚠️ 記得它要放在**遞迴呼叫的後面**，那是「往上收」。

到編輯器做這一題：〈練習：算出每個節點的子樹大小〉

### 做一個

一棵樹，邊是 `0-1`、`0-2`、`1-3`、`3-4`、`2-5`。以 0 為根，印出最深的節點和它的深度。

🔴 深度是**一路傳下去的**：`dfs(v, u, d + 1)`。
⚠️ 而這一題不用 `visited` 陣列——記住 `parent` 就夠了，因為樹沒有環。

到編輯器做這一題：〈做一個：最深的那個節點〉
