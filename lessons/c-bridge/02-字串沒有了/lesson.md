...
到編輯器做這一題：〈練習：把 t 改成 char t[3]，看它壞不壞給你看〉

### 做一個

印出 `"Semorphe"` 的**長度**，再把它**一個字一個字倒著印**出來。

🔴 `char` 陣列沒有「反轉」這種東西可以叫——你只能自己用迴圈從後面往前走。
⚠️ 一個字一個字印要用 `%c`，不是 `%s`。

🔴 **而這一題示範了 C 字串真正的代價**：`s` 是一排格子，
「它有多長」這件事**不存在那排格子裡**——要嘛去數那個 `\0`，要嘛自己記著。

到編輯器做這一題：〈做一個：把名字倒著印出來〉
