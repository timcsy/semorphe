...
→ 教訓 [[一條「本機全綠就等於好了」的習慣]]（`knowledge/experience.md`）
...
→ 教訓 [[「記憶體不足」是系統送出的訊息]]（`knowledge/experience.md`）
...
→ 教訓 [[一支在乾淨機器上綠、在忙碌機器上紅的測試]]（`knowledge/experience.md`）

🔴 **e2e 那一列是 2026-08-25 加的，而它是那天最貴的教訓**：一天改了六刀 UI，
每一刀都跑了全套（5600+ 綠）＋ 開瀏覽器實測，**而 e2e 一次都沒跑**
——CI 從那天 00:20 起紅，**19 支**，是使用者發現的。

⚠️ **這一列的範圍**：`npm test` 驗函式，e2e 驗**使用者按得到的東西還在不在**
——那六刀改的正好都是後者。教訓 [[一整天沒跑 e2e]]。
...
⚠️ **而「寫死了那段文字的東西」包含課文，它不在 `npm test` 裡**——這一句界定的是
這一列涵蓋到哪裡，所以它留在這裡。教訓 [[一次「輸出的文字表示變了」的改動]]。
...
→ 教訓 [[本機那一台比 CI 那一台寬鬆的地方]]（`knowledge/experience.md`）
...
⚠️ 常駐的 `audit-lesson-vocabulary` 與 `audit-solution-vocabulary` **都跑了、都綠**：它們量的是課文的片段與解答檔，
而 e2e 那一條掃的是**整份課文的每一段程式碼**。

→ 教訓 [[兩個名字很像的檢查]]（`knowledge/experience.md`）
...
→ 教訓 [[一份產物的過期有兩種來源]]（`knowledge/experience.md`）
...
→ 教訓 [[一個「零缺陷」的讀數，與一個「零樣本」的讀數]]（`knowledge/experience.md`）

處方：重跑前先 `find $STUDYCPP_DIR -name "*.cpp" | wc -l`，不是 218 就重新 clone。
🟢 而**形狀收進來了**：護欄 `declaration-shapes-roundtrip`
（53 條，`npm test` 會跑）。

第一次跑量到的東西：**殘差 0.0%，而 218 支裡 144 支轉一圈回來不是同一棵樹**。

→ 教訓 [[殘差量的是「我沒認出來」]]（`knowledge/experience.md`）
...
→ 教訓 [[一個只錯在 execute 那一路的缺陷]]（`knowledge/experience.md`）
...
→ 教訓 [[一個量測工具，在它自己被量過之前]]（`knowledge/experience.md`）
...
而 `audit-refcc-headers` 盯著還在手列的檔數。
...
🔴 **教訓的家在 `knowledge/experience.md`**（使用者 2026-09-26 拍板，
見 [history/290](knowledge/history/290-CLAUDE-md-與-memory-是快取而教訓的家在-experience.md)）。
這一份與 `~/.claude/.../memory/` 都是**快取**，而快取有三個機制：

```
flush        新學到的教訓寫進 experience,這裡只用 [[指名]] 引用
invalidate   指名解析不到 ⟹ audit-knowledge-links 紅（那就是失效機制）
policy       進得來的資格 ＝【它必須在一個我認得出來的時機被想起】
             一條要「先想起去查」的教訓,快取它沒有用 —— 它進 experience 就好
```

⚠️ **所以這裡不放蒸餾過的引言**（`> **…**`）——那是硬性零，護欄在擋。
留在這裡的是**操作**與**觸發條件**：「改了 X → 跑 Y」、「這一列涵蓋到哪裡」。

⚠️ 而 `memory/` 在使用者的機器上、每人一份，**護欄永遠看不到它**
⟹ 它只放**指標**，不放內容。
