...
/**
 * 🔴 **這個檔的 273 支測試可以被分到不同機器上跑**（2026-09-14）。
 *
 * ## 它為什麼值得一行設定
 *
 * 量到的：e2e 分成四片之後，四片的時間是
 *
 * ```
 * e2e (1)  1580 秒（26.3 分）  ← 幾乎是全部
 * e2e (2)    69 秒
 * e2e (3)    77 秒
 * e2e (4)   295 秒
 * ```
 *
 * `--shard` 在 `fullyParallel: false` 之下**以檔為單位**分配，而這一個檔
 * 佔全庫 420 支裡的 **273 支（65%）**——它整包落在第一片，**分片什麼都沒買到**。
 *
 * > **一個分片的加速上限是它最大的那一塊；
 * > 而當那一塊【就是全部】的時候，上限等於沒有分。**
 *
 * ## ⚠️ 它不觸犯 `playwright.config.ts` 的 `workers: 1`
 *
 * 那一行擋的是「**同一台機器**上四個 Chromium 擠四個效能核」（量出來的，
 * 見該檔檔頭）。`mode: 'parallel'` 在這裡只改變**分片的顆粒度**
 * ——從「整個檔一起走」變成「一支測試可以單獨走」——
 * 而每一片**仍然是 `workers: 1`**，一次跑一支。
 *
 * ## 🟢 而這 273 支【本來就是】獨立的
 *
 * ```
 * module 層的 let／var    0
 * beforeAll／beforeEach   0
 * describe.serial         0（全庫 43 個 e2e 檔，一個都沒有）
 * 每一支的起點            freshApp（清 localStorage）＋ 自己的 page fixture
 * ```
 *
 * > **這一行做的事是【告訴 Playwright 它們是獨立的】，
 * > 不是【讓它們變成獨立的】。**
 *
 * ## 🟢 實測（2026-09-14，本機四片全跑過）
 *
 * ```
 *         分片前            分片後
 * 支數    369/17/18/16      105/105/105/105   ← 420 支一支不少
 * 時間    26.3 分（第一片）  6.8 / 3.2 / 3.4 / 4.9 分
 * 結果    全綠              全綠（第 4 片 104 過 ＋ 1 跳過＝那支缺輸入檔時自己跳）
 * ```
 *
 * **關鍵路徑 26.3 分 → 6.8 分。**
 *
 * ⚠️ **而「跑一輪」在這裡就是那個有意義的驗收**：這一輪之後，每一支測試都
 * 在**沒有它原本鄰居**的那一片裡跑過了——順序相依如果存在，它在這一輪就會現形。
 * （`workers: 1` 之下同一片內仍然是依序跑，所以這一行改變的是「誰跟誰同機」，
 * 不是「同時跑」。）
 */
...
for (const c of CASES) {
  test(`★ ${c.name}：宣告的元件 ＝ 程式碼真的用到的元件`, async ({ page }) => {
    ...
    // 🪦 **「宣告了而用不到」那一條 2026-09-07 搬走了**（spec 178）。
    //
    // 🔴 **它與這一支量的東西對不上**：這裡量的是「**完成的樣子**」那一段，
    //    而 `components` 要涵蓋的是**整篇課文**教到的積木。
    //    第 2 課的課文講了 `//`（註解），而「完成的樣子」裡沒有它
    //    ——於是這一條把一個**正確的宣告**報成「多開的雜訊」。
    //
    // > **兩條檢查說著同一句話（宣告 ＝ 用到），而它們對「用到」的語料不同
    // > ——那時比較嚴的那一條會擋住比較對的那一份宣告。**
    //
    // 🟢 它搬到 `audit-lesson-vocabulary`（第一百一十四條）——那一支讀的是
    //    **課文裡每一段程式碼**，而不是只有最後那一段。
  })
