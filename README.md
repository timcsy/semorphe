...
<p align="center"><sub>
  <b>Arduino IDE 2.3.8</b>——左邊是 <b>Arduino 自己的編輯器</b>，中間與右邊是
  <b>IDE 原生的編輯器分頁</b>；三邊是<b>同一個 <code>.ino</code> 檔</b>，即時同步
</sub></p>

積木工具幾乎都是**另一個 app**：你在它那裡拼、匯出程式碼、再貼回 IDE
——**單向，而且離開它就回不去**。
...
| | |
|---|---|
| **你自己的檔** | 那是 Arduino IDE 開著的 `sketch_sep3a.ino`，不是我們的沙盒裡的一份拷貝 |
| **IDE 的原生分頁** | 「Semorphe 積木」「Semorphe 流程」是**編輯器分頁**（拖到哪一欄都行），主控台與變數在 **panel 那一排**——不是一個浮在上面的視窗 |
| **三邊都能改** | 改積木 → `.ino` 當場變；改 `.ino` → 積木與流程當場變 |
| **編譯與燒錄照舊** | 左上角那兩顆 ✓ → 還是 Arduino 自己的——我們沒有接管它，也不想 |
| **同一份延伸模組** | VSCode 與 Arduino IDE 共用，因為 Arduino IDE 2.x 是 Theia（吃 VSCode 的延伸模組） |

⚠️ 而它不是「把我們的畫面塞進去」：那條狀態列上的
`同步中（vscode-code-view）` 是真的——**文字那一側的真相是 IDE 的文件**，
我們只送**範圍編輯**回去（見〈IDE 延伸模組〉）。

> **我們沒有找到第二個這樣做的工具**——在官方 IDE 裡、對使用者自己的檔、雙向。
...
（畫面見上面〈[在 Arduino IDE 裡面，對你自己的 `.ino` 檔](#-在-arduino-ide-裡面對你自己的-ino-檔)〉。）
...
- **改了規範而沒有機械化的檢查** — 見 `knowledge/skills/build-guardrail`。
  這個專案有 **100 條護欄**（數字由 `tests/integration/audit-guardrail-count.test.ts`
