...
<p align="center">
  <img src="assets/demo.gif" width="820" alt="打一段 C++ → 程式碼、流程、積木同時長出來 → 三邊各改一次，另外兩邊當場跟著變 → 按執行，主控台印出來">
</p>
...
<p align="center">
  <img src="assets/demo-raw.gif" width="820" alt="貼一段含 goto 的 C++ → 認不出來的那兩行變成灰色積木，原文一字不動地放在裡面 → 在別的地方改一個數字，灰色那兩塊原封不動">
</p>
...
- **改了規範而沒有機械化的檢查** — 見 `knowledge/skills/build-guardrail`。
  這個專案有 **98 條護欄**（數字由 `tests/integration/audit-guardrail-count.test.ts`
