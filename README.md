...
<p align="center">
  <img src="assets/demo.gif" width="820" alt="打字 → 積木長出來 → 在積木上改一個字 → 程式碼跟著變 → 切到流程，同一支程式">
</p>

<p align="center"><sub>
  打字 → 積木長出來 → <b>在積木上改一個字</b> → 程式碼跟著變 → 切到流程，還是同一支程式
</sub></p>
...
- **改了規範而沒有機械化的檢查** — 見 `knowledge/skills/build-guardrail`。
  這個專案有 **93 條護欄**（數字由 `tests/integration/audit-guardrail-count.test.ts`
