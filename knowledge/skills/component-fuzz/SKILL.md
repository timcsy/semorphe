...
難度對應 Topic 層級樹深度：easy = depth 0、medium = depth 1、hard = depth 2+（同 `/component-pipeline` 階段四）
...
**IMPORTANT**: Do NOT include any field describing expected components, AST structure, or internal tool behavior. You are writing programs as a teacher, not analyzing tool internals.
...
結果分類基礎定義見 `/component-roundtrip` 步驟六。本 skill 額外加入 `EXPECTED_DEGRADATION`。
...
| **WRONG_COMPONENT** | 語義樹使用了錯誤的概念（如用 `var_declare` 而非專屬概念） | **BUG** — 元件身分違規，積木無法正確渲染 |
...
1. **PASS 的測試**：在 `tests/integration/` 建立測試檔（檔名 `fuzz-{lang}-{scope}.test.ts`），將 PASS 的程式轉化為 round-trip 回歸測試。每個測試**必須同時驗證**：
   ...
   - **元件身分（COMPONENT_IDENTITY）**：語義樹中被測概念使用了正確的 componentId，而非退化到通用概念。使用 `findComponents(sem, 'expected_component_id')` 斷言節點存在。此驗證防止 lifter 用錯誤概念但碰巧生成正確程式碼的假陽性（如指標宣告被 lift 為 `var_declare` 但 roundtrip 仍正確的情況）。
...
**發現 bug 就必須修復，或嚴格註記以確保後續必修。**

測試的目的是找出問題並解決問題，而非只是記錄問題。以下規則為強制性：

1. **能修的立刻修**：如果 bug 的根因已定位且修復不會超出當前範疇，**必須當場修復**，不得只留 `it.todo` 交差。修復後將 `it.todo` 轉為真正的 `it(...)` 測試並驗證通過。
2. **不能立刻修的必須嚴格註記**：如果 bug 涉及尚未實作的概念（如 struct 尚未支援）或需要大規模重構，才允許留為 `it.todo`，但必須：
   - 在測試註解中寫明**為什麼不能現在修**和**何時應該修**（例如「等 struct 概念實作後」）
   - 在報告的「待修復 Bug」區段中列出，附上根因分析
   - 標記為阻擋項目，確保相關概念的 pipeline 執行時**必須回頭啟用這些測試**
3. **測試檔不可少**：即使所有 bug 都修復了，`it.todo` 對應的測試案例程式碼仍需保留在測試檔中——只是從 `it.todo` 改為正式的 `it(...)`。任何被測試過的程式都必須有對應的永久測試案例，**絕對不允許「發現問題但不留測試」的情況**。
4. **零 todo 為目標**：每次 fuzz 結束時，報告中必須明確列出 `it.todo` 的數量和原因。目標是 **零 todo**——除非有上述第 2 點的正當理由。
...
```
🏁 SKILL_COMPLETE: component-fuzz | {lang} | {scope} | {N} 程式 | PASS: {P} | BUG: {B} | TODO: {T} | 測試檔：{test_file_path}
```
