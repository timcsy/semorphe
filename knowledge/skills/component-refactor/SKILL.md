...
description: >
  ...
  雙重註冊、死概念，並修復缺失的產出物。⚠️ **審計的主力是那批護欄**
...
`tests/integration/audit-*.test.ts` **全部**，而 `npm test` 就會跑完。
⚠️ **這裡不寫數字**——寫了的話每蓋一條護欄就要改三個地方，而**沒有人會記得**
（2026-08-14 的 judge 抓到三處寫著「40 條」而實際是 41）。
**能住在結構裡的東西不該寫進說明書**（`knowie` core 逐字）。
...
3. **產生缺失的產出物**——遵循 `/component-generate` 的規則
4. **驗證**：
   ```bash
   npx tsc --noEmit  # 型別檢查
   npm test          # 測試不能破壞
...
**修復產出物的品質要求**（與 `/component-generate` 相同）：
...
```
🏁 SKILL_COMPLETE: component-refactor | {lang} | {mode} | 修復：{N} | 完備率：{before}% → {after}% | 信心合規：{PASS/FAIL}
```
