...
**前置條件**：此 skill 只能在 component-discover、component-generate、component-roundtrip、component-fuzz 都已完成後才能調用（除非使用了 `--dry-run` 或 `--skip-fuzz` 旗標）。
...
對目標概念執行自動化掃描：**五路**（`FIVE_PATHS`，程式碼裡的常數）＋ **測試**。
⚠️ **用語統一成「五路」**——這裡曾經寫「六路掃描（四路 + Execute + Test）」，
而測試不是一路（它不是投影），把它算進去會讓「五路完備」這個詞在三份文件裡有三個意思。
...
對正在整合的概念，產生 5-10 個代表性程式並執行 round-trip 驗證（同 `/component-roundtrip` 流程）。
