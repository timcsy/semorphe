...
### 🔴 而上面那個 28% 是錯的，隔一刀就被自己的量測推翻（第 250 刀）

把那個 `continue` 拿掉實跑：**237 支紅**，而紅的裡面有 `cpp:arithmetic`、`cpp:array_at`
——那些在產品裡一定產得出碼。⟹ **那 237 是量測工具自己的**：`buildDummyNode(spec)`
造的合成節點餵不了手寫產生器需要的屬性。**那個 `continue` 不是在藏缺陷，是在避開 harness 的能力邊界。**

而把**另一個判定者**數進來之後（膠囊自己的 `*.test.ts`，128 顆提到 `generate`）：

```
① roundtrip-all 的 codeTemplate 那一族   101
② 膠囊自己的測試檔                      128
   聯集                                215 / 349（62%）
🔴 兩邊都沒有的                          134 —— 它們【只被語料驗到】
```

⟹ **正確的說法不是「七成沒被驗」，是「134 顆沒有【逐元件】的產碼判定者」**
——而語料驗的是「這一段程式碼產得出來」，不是「這一顆元件產得出來」。

> **一個「覆蓋率 28%」的讀數，可能只是【我只數了其中一個判定者】。
> 而那與「七成沒被驗」在數字上一模一樣。**

⚠️ 這是「那 N 裡有幾個是量測工具自己的」在**同一張表上第二次**發生
——第一次是 evidence 填錯（本則上半），第二次是**我為了證明它而做的那次量測**。
