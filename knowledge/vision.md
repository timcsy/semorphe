...
- [ ] 🔴 **那 44 筆**：`block-registrar` 33 · `core/block-input-names` 9 ·
      `core/projection/block-renderer` 2
      —— ⚠️ 主體是 `block-registrar` 裡 **40 顆命令式的 cpp 積木定義**，
      **Python 一定要動它**。這是一個獨立的大工程，不在第 0 刀裡。
