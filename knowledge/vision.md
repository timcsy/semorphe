...
> **語言無關性目前是破的**——核心層 **20 個檔**硬編語言專屬元件身分（**174 筆**，其中 134 筆在 `src/interpreter/`），違反 P9。同時碎裂（175 元件平均擴散 8.5 檔）與殼（5 路中 **76 個**路徑是殼、5 個缺）。**全部是 `tests/baselines/` 的實測值，不是估計**；清償排在階段 6.5，已開始下降（見下方 053）。
...
| 病 | 症狀 | 誰抓得到 | 基線 |
|---|---|---|---|
| **碎裂** | 東西散在太多地方 | 靜態計數 | 平均擴散 8.5 檔；中立性 20 檔／174 筆（起始 216） |
| **殼** | 東西在，但是空的 | 執行式單元審計 | 🈳76 個路徑 ／ ❌5 個（起始 92／5） |
