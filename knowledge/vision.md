...
> | **F** | **膠囊搬家** ⬅ **進行中 2/177** | 依賴全部 ✅ | 第一顆 `cpp:vector_declare`（8 個落點 → 0）、第二顆 `cpp:char_is_alpha`。步驟已 skill 化：`knowledge/skills/component-encapsulate`。**109 顆的估計已合併**（`io.ts` 那 68 顆與 `strategies.ts` 那 41 顆是同一形狀，見 [history/043](../knowledge/history/043-第二顆膠囊把109顆的估計合併了.md)） |
>
