...
> | 4.5 | 🔴 **`syntax_error` 從殘差通道搬回診斷通道**（2026-08-14 發現，**排在⑤之前**） | 少一個分號今天是 `MarkerSeverity.Info` ＋ `semorphe-residual`，**不走 `onDiagnostics`**——驗收④ 的整套一點都沒套到**第一週最常見的那個錯**上。⚠️ 而它也是⑤的前置：今天分號不在分母裡，「涵蓋率」量的是一個**不含最常見錯誤**的集合。見 [history/062](history/062-把使用者的錯歸進了我們的殘差.md) |
