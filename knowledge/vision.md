...
- [ ] **進行中**——**12 → 11 → 🟢 1**（2026-08-18）。
  剩下的那一筆是 `cpp:var_declare` 的 `declarators`。

> 🟢 **那一跳不是清償來的，是【第四十六條護欄】順帶的**：
> 「投影遺失」護欄（宣告的接點在積木上有沒有落點）逼出八個補插槽的修法，
> 而符合性從相反方向量同一件事，於是一起降到 1。
>
> ⚠️ **而最後那一筆有一個矛盾要先查**：`slot-roundtrip-first-week` 的
> US2（`int a, b, c` 的其餘變數不得消失）**今天是綠的**，
> 而符合性說 `declarators` 走完 render → extract 之後不見了。
> 🔴 **兩條護欄的答案不一樣**——照下面那條規則，**先問哪一條在說謊**，
> 不要直接去修。
