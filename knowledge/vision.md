...
#### 階段 6.16：零件積木 —— 學生看得到零件，而程式碼還是 Arduino

**✅ 使用者拍板（2026-08-18，五項全定）**：形狀走「**一顆積木 ＋ 一個腳位插槽**」，
`pinMode` 由「自動長出一顆**看得見的**積木」負責，第一批六個零件；
辨識不出來時**退回原始積木打底**（可選地問學生一次，而確認後**回寫程式碼且先問過**）；
🔴 **關鍵字表要等語料擴到 ~80 段才定**——今天的 20 段是一個模型一次生成，
用一次抽樣決定一個長期介面代價不對等。

> **為什麼是它**：使用者「**最近教學急著用**」。而它同時服務兩個族群
> ——初學者直接拉積木、學生貼 LLM 生的程式碼——
> 🔴 而判準是**兩邊必須做出同一棵樹**，否則那是兩個系統。
...
✅ 第 0 批（2026-08-18 完成，spec/branch `141-cpp-arduino-builtins`）
         ...
         🔴 而 fuzz 順手抓到並修好【兩個既有的 round-trip bug】——見 experience
            「一個『每次都多一點』的錯誤」
✅ 第 1 批（2026-08-18 完成）—— 🔴 **六個零件收斂成【兩顆】概念**
         身分：pin_attach（接線）· ultrasonic_trigger（觸發序列）
         而 LED／按鈕／類比感測**不需要新概念**：零件是接線積木上的一個
...
- [x] 第 1 批：拉一顆「接上 LED 到腳位 13，叫它 ledPin」→ `setup` 裡**自動長出一顆
      看得見的** `pinMode(ledPin, OUTPUT)` 積木。🔴 **瀏覽器實測過**（2026-08-18）
      ——機制是「膠囊宣告伴生積木、核心不認得任何積木型別」，護欄
      `companion-blocks.test.ts` 驗它接不接得上
- [x] 🔴 **貼上一段有 `ledPin` 的程式碼 → 積木上的插槽是【變數】而不是數字**
      瀏覽器實測：`#define TRIG_PIN 9` ×2 ／ `const int BUZZER_PIN = 8` ／
      `int ledPin = 13` 四種寫法**全部**認成接線積木，零件各自正確，
      而**寫回去時形式一字不差**（不記形式的話會靜默改掉學生的檔案）
- [x] 🔴 **盲測抓到並修好**：只認 `const int` 的話，41% 的腳位宣告轉不動
      （`#define` 10 筆／裸 `int` 3 筆／`const int` 19 筆）
- [x] 進階／腳位分類**仍在**（Sc1 認知鷹架可退場）——沒有新開「零件」分類，
      因為零件變成了參數。那條軸要等「下拉選單裝不下」才會出現
- [x] C／C++ 三個既有目標不得退步；中立性 `total: 0` 不動

⚠️ **明確不做**：物件機制、板子視圖（history/077 已推遲）。
✅ **關鍵字表已定**（2026-08-18）：從 100 段語料量出來，判準是「這個名字**有沒有被當腳位用**」
而不是名字長得像不像——命中率 90.2%，認不出來的 11 個是成系統的一族（`ENA`／`IN1`／`IN2`
這類馬達驅動的慣例名）。表在 `runtime/device-names.ts`，探針 `device-name-recognition.test.ts` 量著它。
🟢 **而型別辨識那一列可以先做**（`Servo`／`DHT`／`LCD` 靠型別名，不需要關鍵字表，
零過擬合風險）——它排在第 2 批。
