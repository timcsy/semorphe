...
- [ ] 🔴 **剩下的 33 筆**：`block-registrar` 裡 **40 顆命令式的 cpp 積木定義**
      —— 🟢 **它等到了**（spec `160`，2026-08-20）：症狀是具體的
      ```
      appendValueInput('EXPR' + idx)    ← 可變參數的機制，命令式、只認 cpp
      print(x)     🟢 固定單參數，宣告式就走得完
      print(a, b)  🔴 可變參數，走不完
      ```
...
- [x] 🟢 **第一條跨語言的等價邊**（spec `156`，2026-08-20）
      → [107](history/107-第一條跨語言的等價邊而契約沒有語言建構中這個狀態.md)
      ```
      🟢 跨語言   ioRole=print：cpp:print、cpp:print_formatted、cpp:serial_print、python:print
      🔴 分歧點   ioRole=input：只有 cpp
      ```
      🔴 **而最大的發現是一個契約缺口**：`skipPaths` 的理由詞彙只有三個，
      **沒有一個是「還沒做」**（刻意的）——**膠囊契約沒有「一個語言正在建構中」這個狀態**。
      那三路記成**誠實的缺**（完備性基線 0 → 3），**不洗成設計**。
- [x] 🟢 **第一趟行為證據**（spec `157`，2026-08-20）
      → [108](history/108-讀了而不驗而等價升級成行為時抓到三個假綠.md)
      `print("hi")` 從**真的 Python 原始碼**走完 lift → 樹 → generate，**一字不差**。
      🔴 而它抓到三個假綠（護欄誤報五筆／roundtrip 走的是降級路徑／只釘了「會報」）
      與一個真的核心缺陷（**降級節點在運算式位置吐出語句式換行**——
      積木那側早就修過，而產生器這側沒有人接）。
      ⚠️ **wasm 仍不出貨**（`tests/assets/`）——沒有 Python target 就沒有人去要它。
- [x] 🟢 **Python code ↔ blocks roundtrip 成功**（spec `160`，2026-08-20）
      → [111](history/111-Python的第一顆積木撞出九筆假設.md)
      **兩條到達路徑都在瀏覽器裡走通**（工具箱拿得到 ／ 貼程式碼出得來），wasm 出貨。
      🎯 **而它撞出九筆核心對 C++ 的假設**（清單在 `specs/160-python-first-block/`），
      其中第 8 筆**就是下面那 33 筆**——`block-registrar` 的可變參數機制。
