...
> | **C1** ⬅ **進行中** | **參數規格化**（`properties: string[]` → `ParamSpec`） | ~~B~~ ✅ | **消費者已交付**（第二十三條護欄，`specs/102-param-spec`），基線 13 筆只准下降。第一個戰果：逼出 `cpp_ifdef` 的分歧（產生器讀 `name`、執行器讀 `condition`）。<br><br>⚠️ **卡點**：`properties` 驅動 `PatternExtractor.deriveRenderMapping`，所以改宣告會改行為（兩次實測、兩次還原）。<br><br>**前置已開工**：第二十四條護欄「推導一致性」已交付，並修掉它抓到的第一個缺陷——區塊註解的內容在積木→語義樹會掉。**剩下的前置是把 107 筆推導固化成顯式宣告**，之後 `deriveRenderMapping` 可刪，`properties` 才自由 |
