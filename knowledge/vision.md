...
| **5b** 🟠 | **這在測什麼**：VSCode 允許同一個設定**依檔案語言給不同的值**。我們把 `semorphe.target` 宣告成 `language-overridable`，並用 `{uri, languageId}` 當範圍去讀它——**所以 `.ino` 與 `.cpp` 可以用不同的工具箱**（教學現場真的會同時開兩種）。<br>⚠️ 這一條驗的是**我們有沒有正確使用宿主的機制**，不是宿主本身。<br><br>**按**：① 看右下角狀態列，`.ino` 開著時顯示的語言名（可能是 `arduino`／`cpp`／`ino`）② 設定 JSON 寫 `"semorphe.target": "arduino"` ＋ `"[那個語言名]": {"semorphe.target": "cpp-beginner"}` ③ 重開面板 | 🟢 `.ino` 的面板變成 **cpp-beginner** 的工具箱（底下狀態列跟著變），而 `.cpp` 仍是 arduino<br>🔴 壞的是覆寫被忽略——兩種檔案都用同一個 |
