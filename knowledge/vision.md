...
#### 🟠 spec 140 的五條：**逐條的按法**（擴充 0.8.2，⚠️ 先重開視窗）

> 🔴 **這五條沒有一條測得起來**：它們全是「人按了之後看到什麼」。
> 而規則是**不得因為這一刀交付了就自己勾掉**。

| # | 按什麼 | 看什麼 |
|---|---|---|
| **1** | 開一個 `.ino`、開面板、**改一顆積木的欄位**，然後在**程式碼那一格**按 `⌘Z` | 🟢 一次還原**整個**改動；🔴 壞的形狀是要按好幾次才回到改動前 |
| **1b** | 把一顆積木**拖到別的位置**（不改任何欄位），再按 `⌘Z` | 🟢 **什麼都不該發生**（拖動不進 undo）；🔴 壞的是它吃掉你上一個真的改動 |
| **2** | **點一顆積木** | 🟢 程式碼那一格對應的行**變色並捲到看得見**；🔴 壞的是沒反應或捲錯行 |
| **2b** | 在程式碼裡**把游標移到某一行** | 🟢 對應的積木被選取（有外框）；🔴 壞的是選到別顆或沒反應 |
| **3** | 按 `▶ 執行` 旁的**單步（→，F10）**連按幾次 | 🟢 積木**依序**高亮；🔴 壞的是全部一起亮、或第一顆之後就不動 |
| **4** | 切到別的分頁再切回來 | 🟢 **捲動位置與縮放還在**；🔴 壞的是回到左上角或縮放重置 |
| **5** | `settings.json` 加 `"semorphe.target": "arduino"`，重開面板 | 🟢 面板用 Arduino 的工具箱 |
| **5b** | 改成 `{"[arduino]": {"semorphe.target": "cpp-beginner"}}`，開一個 `.ino` | 🟢 語言覆寫生效（`.ino` 用 cpp-beginner）；🔴 壞的是覆寫被忽略 |
