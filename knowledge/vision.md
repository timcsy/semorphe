...
- [ ] 🆕 **板子的具名常數在 lift 那側是什麼身分**（2026-08-19，spec `148` 掀出來的）
      —— 貼上 `pinMode(D1, OUTPUT)` → `D1` 成為 `cpp:var_ref`，
      而它的下拉列的是**工作區變數**（一個 `x`）。
      🔴 **今天執行期已經在打補丁**：`var_ref/execute.ts:67` 查不到宣告時才問板子的常數表
      ——**那是誠實降級，而它讓積木上的投影說不出這個名字是什麼**。
      ⚠️ 這是 P2 概念代數的決定（一個名字的身分由誰決定），**不是一個下拉的決定**。
