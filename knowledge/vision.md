...
> | **C1** ⬅ **進行中** | **參數規格化**（`properties: string[]` → `ParamSpec`） | ~~B~~ ✅ | **消費者已交付**（第二十三條護欄「參數規格一致性」，`specs/102-param-spec`）：用 TS AST 判定「誰讀了哪個參數」，基線 13 筆只准下降。它的第一個戰果是逼出 `cpp_ifdef` 的分歧——**產生器讀 `name`、執行器讀 `condition`**，而辨識器兩個都寫所以沒人發現。<br><br>⚠️ **剩下那一半卡住了，而卡點是新資訊**：`properties` **不是描述，是驅動抽取的資料**（`PatternExtractor.deriveRenderMapping` 拿它比對積木欄位名）。所以「把 124 顆宣告改成符合實際」會改變行為——實測兩次改動、兩次來回轉換變紅、兩次還原。<br><br>**下一步不是繼續改宣告，是先把 `deriveRenderMapping` 從 `properties` 解耦**，否則每一筆宣告改動都要驗行為 |
