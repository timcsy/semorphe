...
🟢 **全部驗完**（2026-08-19）。⚠️ 而其中一條（5b 語言覆寫）是**刪掉**的不是通過的
   ——理由見下方的墓碑。
```

#### 🟢 spec 140 的驗收：**逐條的按法**（擴充 **0.9.4**）——七條全過，一條刪除
...
🟢 **七條全部已驗**（2026-08-19，使用者在 Arduino IDE 逐條實測，擴充 0.9.4）。

🪦 **原本還有一條 5b（語言覆寫），使用者決定刪除。** 留在這裡的是它的**理由**：
它宣稱的目的是「`.ino` 與 `.cpp` 用不同的工具箱」，⚠️ **而那個前提在純 VSCode
裡不成立**——`.ino` 的語言 id 就是 `cpp`，覆寫分不出兩者。
機制本身（`language-overridable` ＋ `{uri, languageId}` 範圍）仍在、仍然正確，
**只是沒有一條人工驗收在守它**。
🔴 想在 VSCode 裡分開，得自己貢獻一個 `.ino` 語言——**會接管語法高亮、
可能與官方 Arduino 擴充打架**。沒有做，也沒有決定要做。
教訓已蒸餾進 `experience.md`「一個『依語言』的設定，能不能分辨兩種檔案，是宿主決定的」。
...
⚠️ 當時的規則（留著給下一張清單用）：**哪一條壞了就說哪一條**，不必全部按完；
**沒按的那幾條維持 🟠**，🔴 **不得因為其他條過了就一起勾掉**。
🟢 這一張是逐條按出來的——七條 🟢 ＋ 一條刪除，沒有一條是「順便勾的」。
