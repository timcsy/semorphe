...
> 🔴 **2026-08-19 逐條對照程式碼，三個缺口只剩一個半。**
> 照舊文走會**重做已經做完的事**——見下表的「實測」欄。

| | 原文 | 實測（2026-08-19） |
|---|---|---|
| ① | ERROR 節點的身分（今天全部降級成 `raw_code`） | 🟡 **字面仍為真，而它拿不到 P6 的背書**。P6 要的是「必須可見」「必須區分原因」，兩條都成立了（`syntax_error` 有自己的顏色 `#FF6B6B`、tooltip、CSS class，並走診斷通道進 Monaco）。**「有身分」買到什麼還沒有人說得出來** → 降級成待論證，不是待實作 |
| ② | 錯誤訊息的投影（已做掉一半） | 🟢 **已關**。20 個錯誤／降級訊息鍵，**值仍是代號的 0 個、缺翻譯的 0 個**；`syntax_error` 給的是句子：「這一段的語法不完整，積木上會少一塊」 |
| ③ | 修復建議（「你是不是要打 X」的候選集合） | 🟡 **一半**。執行期有（`interpreter/near-miss.ts`，`UNDECLARED_VAR_SUGGEST`）；🔴 **語法錯誤那一側沒有** |

- [x] 🟢 **③ 的語法錯誤那一半**（spec `143`，2026-08-19）
      → [092](history/092-把構想翻過來的是一次量測.md)
      ⚠️ **而它做的不是原本寫的那件事**：「你是不是要打 X」被實測否決
      （`whlie` 是合法識別字，那個 token 拿不到），改成解析器**指名**的
      「少了 `;` @ 第 N 行第 M 欄」。重開條件寫在 `specs/143` 的明確排除。

> ⚠️ **這一段被改寫過，理由要留著**：路線圖停在舊的世界觀，於是**把已經修好的東西列成待辦**。
> `experience.md` 有一條「一份指著存在檔案的過時說明書最難發現」——**路線圖正是那份說明書**。
...
- [ ] `arduino-uno`／`arduino-nano`／`esp32` 三個目標選得到，而它們**提供不同的常數與函式**
      - [x] 🟢 **函式那半**（spec `142`，2026-08-19 瀏覽器實測：ESP32 219 顆／
        Uno・Nano 214 顆，剛好差那五顆；Uno 下貼 ESP32 程式碼仍認得，零降級）
        → [091](history/091-能力是一個性狀而不是一份清單.md)
      - [ ] **常數那半**——腳位下拉仍是 Uno 的。見下一步「屬性的候選值由目標提供」。

      > 🔴 **這一行 2026-08-19 被改寫過一次，而那次改寫是錯的。**
...
- [ ] **第一步：消掉那個雙重真相**——`properties[].values` 成為下拉選項的唯一來源
      （或反過來刪掉它；**兩個方向都要先論證**，不得默默選一個）
- [ ] 第二步：候選值可以由目標提供
- [ ] 三塊板子的腳位下拉不同
- [ ] 175 顆既有元件的宣告**一顆都不用改**（省略 ＝ 用今天的靜態值）
