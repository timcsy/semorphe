...
> | **C1** ⬅ **下一步** | **參數規格化**（`properties: string[]` → `ParamSpec`） | ~~B~~ ✅ | 欠規格 **122 → 0**（護欄實測值；舊寫的 128 是 B 項合併前的）。⚠️ **必須同時交付一個會因規格寫錯而變紅的消費者**——否則只是把 122 個空殼填滿。**順帶結案 `const`／`constexpr` 的修飾詞要不要變成參數**（B 項判定為「待 C 項」） |
> | **C3** ⬅ **先做** | **id 型別化**（`conceptId: string` → branded `ComponentId`） | ~~B~~ ✅ | `const x: ComponentId = 'typo'` 編譯失敗；**不動任何存檔**（純型別層）。它讓 D 的每一個改動點都被編譯器找出來 |
