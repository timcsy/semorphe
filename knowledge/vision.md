...
> | ~~**C1**~~ | ✅ **參數規格化**（2026-08-08，`specs/102-param-spec`） | ~~B~~ ✅ | 已達成：124 顆有參數的元件全部 `ParamSpec`（`kind`／`values`／`default`／`required`），**每一筆都是量出來的**。<br><br>驗收條件不是「宣告寫得漂亮」而是 SC-004「消費者的報表要變」——結果**三個獨立消費者同時反應**：第二十三條護欄當場 TypeError（它把 `properties` 當 `string[]`）、`synth-node` 從名字正則猜預設值改成讀宣告、第九條護欄的信號 6 因為簽章含 `default` 而整組消失。<br><br>戰果：抓出 `cpp_malloc` 讓使用者產出 `(int)malloc(…)`（不合法的 C++）；4 筆產生器／執行器缺省分歧修掉、3 筆宣告 required；新增四條檢查。<br>⚠️ **推導固化是前置**：186 筆對應顯式化＋推導刪除，因為 `properties` 本來會驅動抽取行為。見 [history/030](history/030-C1參數規格化-證據優先於名字.md) |
> | ~~**C3**~~ | ✅ **引用完備性**（2026-08-07，`specs/101-component-id-integrity`） | ~~B~~ ✅ | 已達成：新增第二十一條護欄「程式碼不得建出登錄表裡沒有的身分」，幽靈 → 0；補齊 `priority_queue`（元件 175 → 177）。<br>⚠️ **機制被 re-route**：原訂 branded type，實作前發現它**攔不到打錯的 id**（`componentId('typo')` 照樣編譯），而實測的四筆幽靈它一筆都攔不到。branded type 降級為「等它有指涉物再說」——真正攔得到的是登錄表導出的字面聯集型別（需 codegen），應在 D 之後評估 |
> | ~~**C2**~~ | ⏸ **資訊軸（邊）** ——**延後，不是取消** | — | **今天沒有消費者**：`concepts/元件.md:343` 說它「尚不存在——這是要加的那條」，而它來自 ArduinoCAD 的元件協定；semorphe 這側四個 draft 沒有一個需要它（`語義診斷系統` 掛在節點上，不是邊上）。而「機制有了沒人接上」在本專案已有**六個實例**——現在加一整條沒人讀的軸就是預約第七個。→ 等硬體域併入，或等它長出消費者 |
> | ~~**D**~~ | ✅ **命名空間遷移**（2026-08-08，`specs/103-namespace-migration`） | ~~B~~ ✅, ~~C3~~ ✅ | 已達成：177/177 顆身分為 `<scope>:<name>`（`cpp` 145／`lang` 32），舊格式引用 3251 → **0**（硬性零，驗過會紅），積木型別指紋 186 顆**不變**，存檔 v2→v3 轉換附測試。<br><br>⚠️ **主教訓：身分不只以字串字面出現**——找到**十類**掃描器看不到的形狀（裸物件鍵／正則／模板字串／型別聯集／測試標題／產出的程式碼文字／JSON 的值位置／AST 夾具／拿形狀當判斷／與積木型別同名）。最兇的是 `if (concept.includes(':')) return`：**把命名慣例當成契約**，慣例一改單步除錯完全失效。<br><br>分段的價值兌現在回退：第 ① 段跑三輪（128 → 62 → 20 → 0），每輪回退整段而不是就地補。見 [history/031](history/031-D命名空間遷移-身分不只以字串出現.md) |
> | ~~**E**~~ | ✅ **登錄表導出**（2026-08-07，`specs/100-registry-derived-lists`） | — | 已達成：工具箱手寫 80 筆 → 45 段有序來源，12/12 分類產出一字不差；新增第十九條護欄「可拿性」，7 顆使用者拿不到 → 0。見 [history/029](history/029-E項登錄表導出-清單不是碎裂是量測算錯了.md) |
> | **F** | **膠囊搬家** | A–~~E~~ ✅, ~~C1~~ ✅, ~~C3~~ ✅, ~~D~~ ✅ | 專屬檔 2% → **100%**；就近性 → 1 |
>
...
>
> ⚠️ **E 原本的驗收標準是錯的**，寫著「前兩大共用檔的元件提及數 → 0」。
> 實測 166／164，**一顆都沒少，而且不該少**——課程清單本來就該提到很多元件，
> 它是登錄表的視圖，不是碎裂。真正錯的是**量測**：就近性把清單算成實作擴散。
> 修法是修量測（清單／實作分開計），不是刪資料。見
> [history/029](history/029-E項登錄表導出-清單不是碎裂是量測算錯了.md)。
>
> **這條假目標活了兩天沒被發現**，因為它是一個**數字**——而數字天生看起來像已經量過。
>
