...
> | **C1** ⬅ **卡點已解除** | **參數規格化**（`properties: string[]` → `ParamSpec`） | ~~B~~ ✅ | **消費者已交付**（第二十三條護欄，基線 13 筆只准下降），第一個戰果是逼出 `cpp_ifdef` 的分歧。<br><br>✅ **前置已完成**：推導從 3 份收斂成 1 份 → 186 筆對應固化成顯式宣告（**合併結果一字不差**）→ 推導刪除。**驗證解耦**：把 `var_declare` 的參數列整個換掉，抽取結果一模一樣。<br>順帶新增第二十四條（推導一致性，抓到區塊註解掉內容）與第二十五條（顯式對應必填）。<br><br>**下一步**：ParamSpec 的種類詞彙 ＋ 六顆小批驗證消費者會叫，再展開到 124 顆 |
