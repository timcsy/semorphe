...
>
> ### ~~F1：積木型別從 conceptId 導出~~ ✅ **已完成**（2026-08-11）
>
> 一個名字取代兩份會漂移的命名。153 顆不導出 → **0**（硬性零，第三十六條護欄）；
> 存檔 v9 → v10，是**這個專案第一次改寫積木狀態**（既有八步都只碰語義樹）。
>
> 交付細節與四類誤傷見 [history/048](history/048-積木型別導出與四類描述過去的東西.md)。
>
>
