...
> | | 項目 | 依賴 | 關鍵驗收 |
> |---|---|---|---|
> | ~~**A**~~ | ✅ **多形態機制**（2026-08-07，`specs/097-multi-form-projection`） | — | 已達成：元件數 181 → 181、積木 77 → 81、十七條護欄零上升。**B 的阻斷解除**。見 [history/027](history/027-A項多形態機制從阻斷變成解除.md) |
> | ~~**B**~~ | ✅ **身分整併**（2026-08-07，`specs/098`／`099`） | ~~A~~ ✅ | 已達成：元件 182 → 175、**確定桶 9 → 0**、要看 43 → 34 ＋已判定 9。撈出四個缺陷、一顆進墓碑。見 [history/028](history/028-B項身分整併-位置不是身分.md) |
> | **C** ⬅ **下一步** | **結構補齊**（ParamSpec／資訊軸／id 型別化） | ~~B~~ ✅ | 欠規格 128 → 0；資訊軸（邊）存在於語義樹。**順帶決定 `const`／`constexpr` 的修飾詞要不要變成參數**（B 項判定為「待 C 項」） |
> | **D** | **命名空間遷移** `<scope>:<name>` | B, C | 全部遷移；**舊存檔自動轉換**（P8 範圍已釐清，見 [history/026](history/026-P8-的範圍不含語義詞彙本身.md)） |
> | **E** | **登錄表與清單導出** | —（可與 C 並行） | 前兩大共用檔（172／170 顆）的元件提及數 → 0 |
> | **F** | **膠囊搬家** | A–E | 專屬檔 2% → **100%**；就近性 → 1 |
>
> **A 排第一是因為依賴倒過來了**：身分健檢報出的 47 筆裡最大的兩類（6 對
> statement/expression、`container_push` 的 stack/queue）**都要靠多形態才能整併**，
> 而 `BlockSpecRegistry.byConceptId` 是 `Map<string, BlockSpec>`，一個概念只能對一個積木。
> 而且**膠囊會固化身分**，所以身分整併本來就得在搬家之前。
>
> **A 同時是垂直切片**，而且跑在一個真實的使用者 bug 上——學生回報 stack/queue 的
> push 積木寫錯了，根因是統一身分時把形態也一起統一了（見
> [episodes/2026-08-07-學生說積木寫錯了](episodes/2026-08-07-學生說積木寫錯了.md)）。
>
> **E 值得單獨講**：它一項吃掉 24% 的檔次，而那**不是碎裂**——課程清單本來就該提到
> 很多元件，它是登錄表的視圖。改成導出是一次性的，不是 342 次編輯。
>
> 設計理由（在建，做完才反流退場）：[draft/2026-08-07-元件目錄與膠囊契約](draft/2026-08-07-元件目錄與膠囊契約.md)
>
> **F 之前要先 skill 化**——「搬一顆元件進膠囊」會重複 181 次，而既有的
