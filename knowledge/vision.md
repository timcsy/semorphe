...
🟠 還沒驗：**3（單步）／4（分頁狀態）／5・5b（目標與語言覆寫）**
   ——使用者 2026-08-19：「其他我繼續測」
```
...
🟢 **1／1b／1c／2／2b 已驗**（2026-08-19，使用者在 Arduino IDE 逐條實測）
——⚠️ **3／4／5／5b 維持 🟠**，使用者說「其他我繼續測」。
...
| **5b** 🟠 | 改成 `{"[arduino]": {"semorphe.target": "cpp-beginner"}}`，開一個 `.ino`。⚠️ 中括號裡要填**右下角狀態列顯示的語言名**——`.ino` 在不同宿主可能是 `arduino`／`cpp`／`ino` | 🟢 語言覆寫生效（`.ino` 用 cpp-beginner）；🔴 壞的是覆寫被忽略 |
