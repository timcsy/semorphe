...
**階段 6.16（零件積木）**——擴充 0.8.1 已裝，⚠️ **而以下只在網頁版與測試裡驗過**：

- [ ] Arduino IDE 裡：新的「零件與模組」分類拿得到、標籤正確
- [ ] Arduino IDE 裡：貼一段用 `LiquidCrystal_I2C` 的程式碼 → 不被改寫成並列版
- [ ] Arduino IDE 裡：拉一顆接線積木 → `setup` 裡長出 `pinMode`
