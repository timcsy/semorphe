...
- [x] 🟢 **先讓它看得見，再清它**（spec `153`，2026-08-20）
      → [104](history/104-語言耦合先變成看得見的再清它.md)
      ```
      視圖 import 語言   3 → 0      其餘 UI 檔   2 → 0
      而「直接呼叫視圖」仍是 67——沒有把耦合換個地方
      ```
      🔴 **而量的時候發現護欄只掃【概念身分】，不掃【積木型別】**
      ——中立範圍裡有 **44 筆**它看不見。已加成第二維（基線 44、只准降）。
- [x] 🟢 **核心層那 11 筆歸零**（spec `154`，2026-08-20）
      → [105](history/105-核心層的積木型別歸零而佔位值差點變成新的靜默降級.md)
      第二維 **44 → 33**；`src/core/` 貢獻 0，而第一維與「直接呼叫視圖」都沒動。
      🔴 那個檔的註解（057 寫的）四十幾支規格之前就標出這個決定：
      「要涵蓋它們，這個模組得引用語言套件，那與『核心不認識語言』相衝……**不硬湊**。」
      而答案不是「涵蓋更多」，是**整個搬走**。
- [x] 🟢 **第三維（import）：1 → 0**（spec `155`，2026-08-20）
      → [106](history/106-護欄的維度是被發現的不是被設計的.md)
      `core/lift/lifter.ts` 曾 import 一顆 cpp 元件——**而兩維都看不到它**
      （它們掃字串字面）。🔴 **而 P9 原文早就寫著這一維**：
...
- [ ] 🔴 **第一個【不】落在同一類的地方在哪**
      —— **它比上一條更有價值：它是鄰域的邊界**
      （已知的第一個候選：`cout << a << b` → `printf` 要**合併**格式字串，
      那不是逐項投影再相加）
- [ ] C++ 既有測試不受影響
- [ ] 語言切換時 toolbox 自動更新
