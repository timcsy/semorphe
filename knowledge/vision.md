...
> | **F** | **膠囊搬家** ⬅ **只剩這一項** | 依賴 A–E ✅、C1 ✅、C3 ✅、D ✅、~~G~~ ✅ | 驗收：專屬檔 2% → **100%**；就近性 → 1 |
>
...
> | ~~修飾詞站主體位~~ | ~~14~~ → **0** ✅ | **0** |
> | ~~主體不在前~~ | ~~8~~ → **0** ✅ | **0** |
> | ~~殘留 `lang:` scope（D1）~~ | ~~32~~ → **0** ✅ | **0** |
>
> 護欄已就位：`tests/integration/audit-naming.test.ts`（第二十七條），
> 詞彙表在 `src/languages/cpp/naming.ts`。**六項現在都是紅的**，
> 而且驗過會紅（注入 `cpp:strdup` → 當場被指名）。
> **已收硬性零**（2026-08-09）——開工時判棘輪的兩個前提（違規量大、修法昂貴）
> 做完後都不在了，而**維持 0 是免費的**。收硬性零有時效：
> G 完成而不收，下一個人加元件時沒有東西擋他，而那時已經有版本，
> 改起來要發版＋遷移＋立墓碑。
>
> **第 2 項已完成**（2026-08-09）：**D1**——`lang:` scope 退場，32 顆 → 0。
> 存檔 v4→v5。`SCOPES` 白名單只剩 `cpp`。
>
> **第 1 項已完成**（2026-08-09）：接收者參數統一叫 `obj`，10 顆 → 0。
> 存檔 v3→v4 遷移（**語義樹是存下去的**，參數改名也要遷移）。
> 回報：`lifters/io.ts` 那張只為了容納不一致而存在的 `METHOD_OBJ_PROP`
> 對應表**整個消失**。瀏覽器實測輸出 43。
>
...
>
> **已定**：分隔符 `_`；scope ≠ 套件；scope 是**套件擁有者**不是語言；
> 積木各語言各自；**D1 已執行**（`lang:` 退場）。
