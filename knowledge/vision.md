...
> | **C1** ⬅ **下一步** | **參數規格化**（`properties: string[]` → `ParamSpec`） | ~~B~~ ✅ | 欠規格 **124 → 0**。⚠️ **必須同時交付一個會因規格寫錯而變紅的消費者**——否則只是把 124 個空殼填滿。**順帶結案 `const`／`constexpr` 的修飾詞要不要變成參數**（B 項判定為「待 C 項」）<br><br>**消費者已經有指涉物了**（2026-08-07 實測）：靜態比對「產生器／執行器**讀了**哪些屬性」與「宣告**列了**哪些」，**約 15 顆對不上**——已抽驗確認為真的有 `var_declare.init_style`（產生器分支用它、宣告沒列）、`array_declare.size`（執行器讀它）、`cpp_increment.NAME`（`properties.name ?? properties.NAME` 的**大寫退路**）。⚠️ 該掃描第一版有假報（區塊切割跨到隔壁產生器，`print.value` 是其一）——**判準要先在已知答案上驗**。<br><br>另有**實例側**的：`cpp_include` 的實例帶 `local`（boolean）而宣告沒列 |
