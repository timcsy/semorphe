...
🟢 **同步那一族已驗**（2026-08-19，使用者逐條實測，擴充 0.9.4）
   ⌘Z 的粒度不差於宿主 · 純移動不進歷史 · 拖積木一次還原到位
   點積木↔程式碼行**雙向**定位
   ⚠️ 途中修掉**六個**真缺陷，而**只有最後兩個**（拖曳中寫入、欄位編輯中寫入）
      是使用者回報那件事的成因——其餘四個是查案途中撿到的
      （見 `history/087`／`088`）

🟠 還沒驗：**只剩 5b（語言覆寫）**
   ⚠️ 而它沒被驗的原因是**條文看不懂**——使用者逐字：「5b 我不知道是什麼意思」。
   > **一條沒有人看得懂的驗收，與一條沒有人按的驗收，結果完全一樣。**
   已補上「這在測什麼、為什麼要有它」。
```

#### 🟠 spec 140 的五條：**逐條的按法**（擴充 **0.9.4**，⚠️ 先重開視窗）

> ⚠️ **版本要對得上。** 這張清單的意義押在版本上——按出來的結果屬於哪一版，
> 是唯一能分辨「修法沒載到」與「修法沒用」的東西
> （2026-08-19 為此在診斷第一行印版本，見 `history/087`）。
> **改版之後這一行要跟著改**，否則下一次實測會對著錯的基準判斷。

> 🔴 **人要按的是「看起來對不對」那一半。**
>
> 原文寫的是「這五條沒有一條測得起來」，⚠️ **那句話在 2026-08-19 之後
> 不再完全成立**：第 1／1b 的**機械核心**已經有護欄在守
> （`tests/integration/bus-update-not-user-edit.test.ts`，12 支）：
>
> ```
> 拖曳中不得寫回，放下之後補一次
> 沒有語義變動時逐字還原成原檔 → 不寫
...
>
> **留著原話會讓下一個人以為完全沒有防線。**
> 而剩下的那一半（人按下去看到什麼）仍然測不起來——
> 規則不變：**不得因為這一刀交付了就自己勾掉**。

🟢 **八條裡的七條已驗**（2026-08-19，使用者在 Arduino IDE 逐條實測，擴充 0.9.4）
——⚠️ **只剩 5b 維持 🟠**（使用者：「5b 我不知道是什麼意思」→ 條文補了「這在測什麼」）。
...
| **5b** 🟠 | **這在測什麼**：設定可以**依檔案語言給不同的值**。我們把 `semorphe.target` 宣告成 `language-overridable`，並用 `{uri, languageId}` 當範圍去讀——這一條驗的是**我們有沒有正確接上宿主的機制**。<br><br>🔴 **已知限制（2026-08-19 使用者指出）**：`.ino` 拿到什麼語言 id **完全看宿主**，我們沒有註冊自己的語言。<br>　`Arduino IDE` → `arduino`（`history/080`§一 查證）→ 🟢 覆寫分得出 `.ino` 與 `.cpp`<br>　**純 VSCode → `cpp`** → 🔴 **分不出**，`[cpp]` 會同時套到兩種<br>　⚠️ 想在 VSCode 裡分開，得自己貢獻一個 `.ino` 語言——**而那會接管語法高亮，並可能與官方 Arduino 擴充打架**。沒有做，也沒有決定要做。<br><br>**按**（在 **Arduino IDE** 裡才驗得出分離）：① 看右下角狀態列的語言名 ② 設定 JSON 寫 `"semorphe.target": "arduino"` ＋ `"[那個名字]": {"semorphe.target": "cpp-beginner"}` ③ 重開面板 | 🟢 面板換成 **cpp-beginner** 的工具箱（底下狀態列跟著變）<br>🔴 壞的是覆寫被忽略，還是 arduino<br><br>⚠️ 在**純 VSCode** 裡這一條只驗得到「覆寫有沒有被讀到」（用 `[cpp]`），**驗不到 `.ino` 與 `.cpp` 分離**——那不是缺陷，是宿主沒有給我們可以分辨的東西。 |

⚠️ **哪一條壞了就說哪一條**——不必全部按完。而**沒按的那幾條維持 🟠**，
🔴 **不得因為其他條過了就一起勾掉**。

> 🔴 **第 1／1b／1c 的前提在 2026-08-19 被換掉了，條文跟著改過一次。**
>
> 寫這張清單時，「一次積木編輯 ＝ 一個復原項」是**設計意圖**。
> 而那一週查出來的是：
>
> ```
> 拖曳過程產生【兩次】寫入        → 已修（0.9.1）
> 每次同步都在重寫整個檔案        → 已修（0.9.2，空行還原後寫入根本不發生）
> ```
>
> ⚠️ 所以 **1b 的「什麼都不該發生」在 0.9.2 之前是不可能成立的**
> ——純移動也會重寫全檔。**用舊條文去驗新版本，會把一個真的修好判成沒修。**
> 新增的 1c 釘的是使用者實際撞到的那個形狀（`history/087`／`088`）。
>
