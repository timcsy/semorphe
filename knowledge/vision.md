...
> **2026-08-05 修正「尚未驗證語言無關性」這句**：原文說語言無關性「尚未驗證，是 Phase 7 的核心目的」。**已經知道它是破的**——`src/core`／`src/ui`／`src/interpreter`／`src/views` 底下有 **20 個檔硬編語言專屬 componentId**（216 筆），直接違反 P9「拔掉 C++…無 `languages/cpp/` import」。
> 同時量到兩個更基本的問題：**碎裂**（175 個元件平均擴散 8.5 檔）與**殼**（175 元件 × 5 路中有 **92 個路徑是殼、5 個缺**）。
> 以上全部是 `tests/baselines/` 的實測值（階段 6.5 P0，2026-08-06），不是估計。
...
>
> 最嚴重的發現：`declaration` 上有 **5 條規則的限定條件完全相同**（都是 `type: template_type`）——第一條贏走全部，**另外 4 條永遠不會被試到**。它們是死規則。
>
> 這條護欄量的是「有多少東西**靠運氣**」，不是「有多少東西壞了」——數字下降代表移除了未來會咬人的機會，不代表修好了 bug。

- [ ] lift 平行機制 **5 套 → 1 條脊椎**（P2–P3；P1 已讓歧義可見）
- [ ] 既有測試全程綠，**一次都不准紅著過夜**（P0 後為 3033 測）
