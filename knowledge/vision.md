...
> **2026-08-05 修正「尚未驗證語言無關性」這句**：原文說語言無關性「尚未驗證，是 Phase 7 的核心目的」。實測後**已經知道它是破的**——`src/core`／`src/ui`／`src/interpreter` 底下有 **12 個檔硬編 C++ 專屬 conceptId**，直接違反 P9「拔掉 C++…無 `languages/cpp/` import」。
> 同時量到兩個更基本的問題：**碎裂**（`cpp_string_at` 散在 13 檔／7 目錄）與**殼**（15/17 個 `std/*/lifters.ts` 是空 stub、64 個 `it.todo`、P2「0 容忍」無任何檢查）。
> → 因此插入 **階段 6.5** 先清償，Phase 7 的角色從「發現架構是壞的」改為「驗證已經修好的架構」。
...
<!--
  ...
  為何插在 Phase 7 之前：Python 會繼承碎裂與殼；且它帶來的是新內容不是新槽，
       而硬體域（未承諾）帶來新槽，兩者都該等格式收斂。見 draft/硬體域併入計畫.md
-->
...
| 病 | 症狀 | 誰抓得到 | 基線 |
|---|---|---|---|
| **碎裂** | 東西散在太多地方 | 靜態計數 | `cpp_string_at` 13 檔／7 目錄 |
| **殼** | 東西在，但是空的 | 執行式單元審計 | 15/17 空 stub |
| **條件性正確** | 單獨測都過，組合才壞 | 組合式測試 | 64 todo ＋ 12+ skip |

**成功標準：**

- [ ] **P0 四條護欄進 CI，各自輸出非零基線數字**，且全部是單調不增的棘輪
- [ ] 新增一個元件觸碰的檔案數 = **1**（就近性；基線 13）
- [ ] 中立性違規 **12 → 0**（＝ P9 語言獨立性第一次真的成立）
- [ ] 完備性報表的 🈳殼 與 ❌缺 **歸零**（＝ P2「0 容忍」第一次真的被檢查）
- [ ] `it.todo` **64 → 0**，`skip` 只剩「墓碑的正確後果」且連到墓碑檔
- [ ] lift 平行機制 **5 套 → 1 條脊椎**（＝ P3「歧義在註冊時仲裁」第一次成立）
- [ ] 3006 個測試全程綠，**一次都不准紅著過夜**
