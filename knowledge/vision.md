...
一個等價類的證據都還沒有。
🟢 **2026-08-22：Python 74 顆，而 95 段 AI 會寫的語料【與真的 `python3` 同一個答案】**
——答案不同／執行失敗／來回不同**三軸都是 0**，而**降級節點也是 0**。
...
| `169` | if 的 elif 鏈與函式參數的 mutation ——三個宣告式建構子 | [120](history/120-一份路線圖寫著缺什麼而那個東西在別的目的下被做出來了.md) |
| `170` | 🔴 **Python 的執行期**——而 17 顆執行器【第一次被跑過】，當天吐出四個真缺陷 | [121](history/121-Python的執行期而十七顆執行器第一次被跑過.md) |
| `171`+`172` | 切語言之後才發現的三處寫死——程式根／語言安裝／變數下拉 | [122](history/122-切語言之後才發現的三處而它們都是同一句話.md) |
| `173`+`cin` | 🔴 **「輸入耗盡」在兩個語言各缺一塊**——Python 的 `input()` 不等使用者且借用了別人的錯誤碼；C++ 的 `failbit` 不黏，於是 `while(cin>>x)` 吞掉壞輸入多跑兩圈 | [123](history/123-一筆判定把半個故事當成全部.md) |
| 護欄的量測機構 | 🔴 **九條護欄記了「我掃了多少」而零條在看**（最大差 46%）；六條掃描型沒有注入；退休兩塊遷移完成後的空殼 | [124](history/124-護欄有了而量測機構自己沒有人量.md) |
| 跑測試的瓶頸 | 🔴 **兩個直覺的分層方案實測都省下 0 秒**——成本是 per-file 的模組載入；預設環境改 `node` ＋ 分層指令 ＋ 結清掛了三次的 flaky | [125](history/125-跑測試的瓶頸不在直覺的地方.md) |
| Python 教學可用 | **AI 生的 Python 20 段語料：執行失敗 12 → 0、來回不同 4 → 0、降級 33 → 0**；元件 24 → 41。而最重要的一軸（跑得動）是量到一半才發現護欄少的 | [126](history/126-還沒支援的邊界會在別的東西做出來的那天變成擋路的.md) |
| 參照直譯器 | 🔴 **三個零合起來也答不出「答案對不對」**——接上真的 `python3` 對答案，累計抓到 **14 筆靜默錯答案**（`round(2.5)`→3、函式裡的指派改到外面、`reverse=` 被忽略…）。判準從「自洽」變成「與第三方同一個答案」 | [127](history/127-驗收改成跟真的直譯器對答案.md) |
| 積木建得起來嗎 | 🔴 **`for` 迴圈在瀏覽器裡一顆都建不出來，而 5312 支測試全綠**——三道防線都停在 `BlockState`（JSON）這一層。第五十一條護欄真的 `newBlock`，中英兩種語系各一遍 | [128](history/128-渲得出積木而渲的是一份JSON.md) |
| 教學可用（收斂） | **73 段語料五軸全零，降級節點也是 0**；元件 24 → 73。而三次「把運算式壓成字串」之後改了做法：指派的左邊是**接點**；巢狀推導式用一個**遞迴的插槽**（層數不受限）而不是「最多兩層」的欄位 | [129](history/129-一個插槽比一個最多兩層的欄位多表達了無限層.md) |
| 積木擺得上去嗎 | 🔴 **程式跑得對而積木一片空白**——`python_method_call` 建得起來，而它宣告的兩格在實例上不存在（可變參數的建構子從零建整顆，表達不了「插槽在前」）。第五十一條護欄長出第二個維度：語料的 95 段各**真的載進一個工作區** | [130](history/130-一段程式的積木擺得上去嗎.md) |
| 工具箱的分類 | 🔴 **`data` 那一格裝了 36 顆，占全部的一半**（使用者回報）。沿用 C++ 的**判準**（按認知意圖分組）重規劃成 12 個分類；第五十二條護欄把「積木顏色 ＝ 它所在分類的顏色」變成可檢查的（Python 硬性零、C++ 棘輪 51） | [131](history/131-一個分類裝了一半的積木它就是其餘.md) |
