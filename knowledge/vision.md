...
> | **F** | **膠囊搬家** ⬅ **進行中 4/177** | 依賴全部 ✅ | 第一顆 `cpp:vector_declare`（8 個落點 → 0）、第二顆 `cpp:char_is_alpha`、第三顆 `cpp:literal_char`。**三種形狀全部驗過**（差別只在 lift 那一路）。步驟已 skill 化：`knowledge/skills/component-encapsulate`。**109 顆的估計已合併**（`io.ts` 那 68 顆與 `strategies.ts` 那 41 顆是同一形狀，見 [history/043](../knowledge/history/043-第二顆膠囊把109顆的估計合併了.md)） |
>
...
> 第一個「只存在於正式路徑」的宣告來源；
> ② **170/177 顆**至少有一路住在共用檔（lift 109 顆、產生器 97 顆、執行器 69 顆），
> 只有 7 顆已在專屬模組資料夾；
