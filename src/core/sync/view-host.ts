...
export interface SemanticUpdateEvent {
   ...
   * 流程是淡的＋拉不動／連不出線／刪不掉）。
   *
   * ## 🟢 兩個視圖都在這條路上了（2026-09-06，spec 172）
   *
   * 積木那一側曾經走組裝點直接呼叫，而那讓組裝點替視圖做了**兩個**決定：
   * 算出哪幾顆是骨架（可以），**以及該畫成 `ghost` 還是 `editable`**（不可以）。
   * 第一百零五條護欄擋著回頭路。
   *
   * ## 🔴 `nodeIds` 是【兩棵樹的聯集】
   *
   * 真相那一棵 ∪ 畫布上那一棵——而單問任何一棵都會壞掉一半：
   *
   * ```
   * 只問真相那棵     增強器補的 #include／using 認不出來
   *                  → 「兩顆實心的積木站在一堆淡的裡面」
   * 只問顯示樹       hidden 下骨架【已經被剝掉了】，於是告示是空的
   *                  → 流程視圖不知道要隱藏誰
   * ```
   *
   ...
   * ## ⚠️ 一則【只帶告示】的更新
   *
   * 鷹架深度變了而樹沒變時，真相那一側發一則**不帶 `blockState`** 的更新
   * （`SyncController.republishScaffold`）。積木面板的重畫閘在 `blockState` 上，
   * 所以那則事件只會套骨架——**不重畫、不打斷拖曳、不動復原堆疊**。
   */
  scaffold?: { nodeIds: string[]; mode: 'hidden' | 'ghost' | 'editable' }
  /**
   * **系統推導出來的那幾顆，這一次變了。**
   *
   * ## 🔴 它存在的理由是一個「兩條路畫出不一樣的畫面」的缺陷（2026-09-15）
   *
   * 使用者回報第 5 課：「`#include` 好像**不會在拉 `cout` 的時候出來**」。
   * 實測到的正是那樣，而**程式碼那一側是對的**：
   *
   * ```
   * 在程式碼裡打 cout   程式碼有 #include <iostream>  積木有那顆淡的  🟢
   * 拉一顆「印出」出來   程式碼有 #include <iostream>  積木【沒有】     🔴
   * ```
   *
   * 根因是積木面板**刻意不重畫自己的編輯**（重畫會打斷拖曳、清掉復原堆疊）
   * ——而自動補的那顆 `#include` **不是它畫的**，是增強器補進顯示樹的。
   *
   * > **一個「不重畫我自己的編輯」的規矩，會把【我沒有畫過的東西】
   * > 一起擋在外面——而那正是只有重畫才出得來的那些。**
   *
   * ⚠️ 所以它是**那條規矩的例外，而不是取消它**：只有在推導出來的那一組
   * 真的變了的時候才為真。實測那在一支程式裡**大約發生一次**
   * （第一次需要某個標頭的那一刻），代價是那一次的復原堆疊被清掉。
   */
  scaffoldChanged?: boolean
  /**
   * **這一次畫出去的那幾顆，是系統推導出來的**（它們的 nodeId）。
   *
   * 🔴 視圖要它，是因為**抽取會把畫布上每一塊重新編號**
   * （`BlocklyPanel.setNodeIdLookup` 至今零個呼叫者，於是「還原原本的
   * nodeId」那一段從來沒跑過）。而推導出來的那幾顆**不在真相樹裡**，
   * 重新編號之後就再也沒有人認得它們——症狀是那顆 `#include`
   * **畫出來了，而它不是淡的**。
   *
   * > **一個不在真相裡的東西，只要它的身分會被下一次抽取改寫，
   * > 那麼任何一份「上一次算好的」清單都會在那之後對不上它。**
   */
