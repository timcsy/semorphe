 ...
 */
export function registerNamedCast(keyword: string, conceptId: string, source: string): void {
  const existing = table.get(keyword)
  if (existing && existing.conceptId !== conceptId) {
    throw new Error(
      `命名轉型「${keyword}」被兩顆元件認領：${existing.conceptId}（${existing.source}）與 ${conceptId}（${source}）。`,
    )
  }
  table.set(keyword, { conceptId, source })
}

/** 這個關鍵字屬於誰。不認得回 `undefined`——**不猜**。 */
export function namedCastConcept(keyword: string): string | undefined {
  return table.get(keyword)?.conceptId
}
