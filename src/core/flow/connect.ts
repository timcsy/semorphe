...
export type RefusalReason =
  ...
  | 'scaffold-locked'
  /**
   * **那一格會變成空的，而這個系統還沒有「空插槽」的表達。**
   *
   * 🔴 2026-08-30 實測：把單一插槽裡的值拿掉，產出的東西**不一定合法**：
   *
   * ```
   * 宣告的初始值  int total;      ✅ 合法
   * if 的條件     if () { }       🔴 壞的
   ...
   * ```
   *
   * ⚠️ 分不出這三種，因為「這一格可不可以是空的」**沒有人宣告過**
   * ——那是 vision 債表裡 `ParamSpec` 那一列（「型別側只有名字清單，
   * 沒有型別、範圍、預設值」）。
   *
   * > **在「可不可以空」被宣告出來之前，
   * > 一個會把插槽清空的操作只能整批擋掉——而擋掉要說得出為什麼。**
   *
   * 🟢 語句不受影響：body 可以是空的，那是合法的 `{ }`。
   */
  | 'slot-would-empty'
