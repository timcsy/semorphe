...
export interface AstConstraint {
  ...
  absent?: boolean
  /**
   * 🔴 **要求這個名字【沒有被宣告過】**（spec 174）。
   *
   * ## 它解的是一份判例
   *
   * `cpp:pin_constant` 的 lift 樣式 2026 年被拿掉，理由逐字：
   *
   ...
   * ——它當時只在**執行期**做得到。這一格讓 lift 期也問得出來。
   *
   * ## ⚠️ 它是一個近似，而那是刻意的
   *
   * 「查不到」＝「沒有人宣告」。作用域的紀錄可能不完整（例如來自標頭的宣告），
   * 而它**偏向安全的那一邊**：查不到時的行為與沒有這一格時相同，
   * **查得到時才改變行為**。
   *
   * 🔴 而它有一條**已知的界線**：判斷用的是 lift 當下的作用域，
   * 而 lift 是由上而下的一趟——所以「宣告寫在使用**之後**」仍然會被搶。
   * 🟢 那在 C++ 裡本來就不合法，所以不是真的使用情境；
   * 解掉它要**兩趟掃描**，而那是另一個設計。
   *
   * > **一個「先問一句再認人」的條件，問的是【問的當下】知道的事
   * > ——而那與「全部知道」之間有一條線，那條線要被寫下來。**
   */
  notDeclared?: boolean
}
...
export interface Declaration {
  ...
  scope: number
  /**
   * 🔴 **這個名字是一個【變數】還是一個【型別】**（2026-09-19）。
   *
   * 在此之前兩者住在同一張表而分不出來，於是 `isKnownType(name)` 只是
   * 「這個名字查得到嗎」——一個叫 `isKnownType` 而答不出「它是不是型別」的函式。
   *
   * 症狀是 `(X)(3)` 這個形狀：它可能是**轉型**（`(ll)(x+1)`）也可能是
   * **函式指標呼叫**（`(fp)(3)`）。分不出來的話只能猜，而**猜錯的方向很重要**：
   * 猜成轉型會讓 `(fp)(3)` 安靜地回傳 3；猜成呼叫會讓它誠實地說「不認得」。
   *
   * > **一個分不出兩件事的判定，它的預設值決定了它錯的時候有沒有聲音。**
   *
...
export interface Target {
  ...
  /**
   * 這個目標在選單裡屬於哪一組——**組名換的時候清單上多一列標題**。
   *
   * 🔴 為什麼是**宣告**不是推導：判準看起來是「有沒有 `board`」，
   * 而 `arduino`（不指定板子）**是硬體卻沒有板子**——推導會把它歸錯組。
   *
   * ⚠️ 2026-08-28 之前這裡沒有這一格，於是目標選單是一列 13 項的平清單，
   * 混著兩個不同的軸（語言／軌道 vs 板子）。使用者看著它問
   * 「這邊能不能重新設計整理一下？」
   */
  group?: string
   ...
   * 組內的順序——**小的在前**。沒宣告的排在最後。
   *
   * 🔴 為什麼不用註冊順序：那是語言套件裡 `import` 的順序，
   * 而它是**依賴關係**排的，不是教學順序。實測 `C++ 進階` 會排到 `C` 後面。
   *
   * ⚠️ 而它也決定了**陷阱排在哪**：`arduino`（不指定板子）是 99，
   * 排在所有板子之後——它不該看起來像預設。
   */
  order?: number
   ...
   * 要不要列在**目標選單**裡。省略 ＝ 要。
   *
   * 🔴 `false` **不代表停用**——它仍然是一個完整的目標，
   * 只是**只由課程進得去**（課的 `pins.target` 指得到它）。
   *
   * 今天有兩個：
   *
   * ```
   * cpp-advanced   它其實是一條【軌道】，不是一個語言
   *                （2026-08-28：「課程可以再拆分成課程和章節，目標可以更單純一些」）
   * arduino        🔴 沒有板子常數的那一個——LED_BUILTIN／A0 都不存在，
   *                而 Arduino 第一課的第一行就用它
   * ```
   */
  listed?: boolean
   ...
   * 選單上跟在名字後面的那句淡色說明。
   *
   * 🔴 它今天只有一個消費者，而那個消費者是**一個陷阱**：
   * `arduino`（不指定板子）沒有 `LED_BUILTIN`／`A0`，
   * 而 Arduino 第一課的第一行就用 `LED_BUILTIN`。
   *
   * > **把一個陷阱標示出來，它就變成一個選擇。**
   */
  hint?: string
