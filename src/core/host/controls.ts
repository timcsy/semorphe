...
/** 使用者拿它做什麼——🔴 **表面由這一格決定**。 */
export type ControlKind = 'picker' | 'action' | 'indicator' | 'output' | 'inspector' | 'identity'
 ...
 *
 * ## 🔴 `identity`：「這一格是誰」也要投影（2026-09-01）
 *
 * 使用者在 VSCode：「有標籤列跟工具列感覺很**厚重**」。而看清楚之後，
 * 厚的那一半是**同一句話說兩次**：
 *
 * ```
 * ┌ Semorphe 流程 ✕ ┐          ← VSCode 的分頁列：這一格是誰
 * │ 流程 ▾  ⤢自動排版  − 100% + │  ← 我們的工具列：這一格是誰 ＋ 動作
 * ```
 *
 * ⚠️ 而積木那一格的列上**只有**「積木 ▾」——整條列為了一個已經寫在分頁上
 * 的名字而存在。
 *
 * 🟢 網頁版**沒有**分頁列，所以那顆在那裡是**唯一的**名字，必要。
 *
 * > **同一顆東西，在一個宿主是必要的，在另一個宿主是重複的
 * > ——因為宿主自己已經說了那句話。**
 *
 * ## ⚠️ 而判準只有一條，不然「越接近越好」會被任何理由推翻
 *
 * 使用者 2026-09-01：「我是希望網頁版和 IDE 版**如果不是非必要越接近越好**，
 * 這樣使用者體驗才好」。
 *
 * > **不畫的理由只有一個：宿主已經做了那件事。**
 * > 不是「這樣比較好看」，不是「省高度」，不是「比較原生」。
 *
 * 🔴 同一天用這條判準**否決了**另一個提案（把流程的自動排版與縮放也搬上
 * 標題列）：VSCode **沒有**提供那些，搬上去的理由只剩「省 34px」
 * ——而那正是這條判準要擋掉的。**它們留在原地，兩邊一樣。**
 */
