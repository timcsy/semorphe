...
/**
 * 每一種控制項投影到哪個表面。🔴 **一個宿主一張**。
 *
 * ## 🪦 這裡曾經有第六種：`identity`（「這一格是誰」畫在哪）
 *
 * 使用者在 VSCode：「有標籤列跟工具列感覺很**厚重**」——而厚的那一半確實是
 * 同一句話說兩次（分頁寫「Semorphe 流程」，我們的列再寫一次「流程 ▾」）。
 * 於是我讓宿主已經說了名字的地方不畫，積木那一格整條列消失。
 *
 * 而使用者看了之後改了主意：
 *
 * > 「我覺得你把積木的工具列**加回來**好了，我覺得**面板統一**好像更重要一點」
 *
 * 🔴 他是對的，而我漏看了一件事：**那三個面板是【我們的】三個面板**。
 * 使用者的眼睛先把它們看成一組，才把其中一個與它的分頁列看成一對。
 * 少掉一條列的那一格，讀起來不是「比較乾淨」，是「**跟別人不一樣**」。
 *
 * > **一致性有兩個方向——與宿主一致，與自己一致。
 * > 而使用者先看到的是後者，因為那三格是同時出現在他眼前的。**
 *
 * ⚠️ 於是那個軸整個退場（兩個宿主都會是同一個值的軸，是雜訊不是宣告），
...
export interface ControlOption {
  ...
  /**
   * 這一項可以畫成一條**兩色的橫條**（2026-09-08，拆輪子的曲線）。
   *
   * 🔴 **它是一面鏡子，不是一句評語**：橫條上**沒有任何一個字**，
   * 它在數字好的時候與不好的時候長得一樣——所以「沒出現」不會變成一種訊號。
   *
   * ⚠️ 而它要有一個**參照系**才有意義（第 1 課全是積木是對的，第 15 課才是訊號）
   * ——參照系寫在 `description` 裡，而那句話說的是**課程**，不是這個人。
   *
   * ⚠️ 宿主不支援畫圖的話忽略它即可——它是**呈現**，不是語義。
   */
  readonly previewBar?: {
    /** 左邊那一色佔多少（0～1）。⚠️ 兩邊都是 0 時**兩個都給 0**——「還沒開始」不是 50/50 */
    readonly left: number
    readonly right: number
    /** 給滑鼠停著與螢幕閱讀器的一句話——⚠️ **只准是數字**，不准是評價 */
