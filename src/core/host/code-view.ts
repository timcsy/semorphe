...
export interface CodeView {
  ...
  /**
   * **把一項偏好交給宿主保存**（2026-09-01）。
   *
   * 🔴 使用者：「**為何出現的是 Arduino 的？跟下面寫的不一樣啊**」
   * ——三個面板的工具箱是 Arduino，而狀態列寫著「C++ 標準骨架」。
   *
   * 根因：在 VSCode 裡**存檔服務刻意什麼都不存**（`DocumentlessStorage`，
   * 因為那裡檔案才是真相）。於是骨架／風格／積木風格這些偏好**沒有家**
   * ——一個新開的面板從預設開始，而舊的那個記著使用者選過的。
   *
   * ⚠️ 單例時代這只是「重開就沒了」；多面板時它變成
   * **三個面板各說各話**，而使用者第一眼看到的就是矛盾。
   *
   * > **一份「不存」的設計，在只有一個讀者的時候看起來像「不需要」
   * > ——多一個讀者，它就變成「他們不會一致」。**
   *
   * 🟢 而它不是回頭去存檔：偏好交給**宿主的設定系統**（workspace settings），
   * 那是這個宿主本來就有、而且所有面板共用的一份。
   */
