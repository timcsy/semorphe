...
export function selectForm(
  ...
  //
  // 🔴 **而上面那兩句只在【軟體域】成立**（2026-09-25 補，拿 3D 域填一遍時發現）。
  //
  //    軟體的組合形態之所以不必要，是因為**標籤可以中性**——「放進」對堆疊與
  //    佇列都讀得通。而 3D 的軸不是這樣：
  //
  //    ```
  ...
  //    ```
  //
  //    ⟹ **軸的組合方式本身是領域相依的**：軟體是「優先序取一個」，
  //    3D 是「笛卡兒積」。而這支函式今天只做得到前者——它**取一個**，
  //    而 3D 要的是**一組座標**。
  //
  // > **一個「刻意不做」的決定，它的理由通常只在做決定的那個域裡成立，
  // > 而它會被寫成一條通則。**
  //
  //    🪦 3D 域進來時這裡要改，而改法不是加一個旗標——是先問
  //    「這條軸是**選擇**用的還是**組合**用的」，那是 `FormAxis` 缺的一欄。
