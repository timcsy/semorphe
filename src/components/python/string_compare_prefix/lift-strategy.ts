/**
 * `python:string_compare_prefix` 的 **lift** 路——認 `x.startswith(…)`。
 *
...
export function registerLiftStrategy(registry: LiftStrategyRegistry): void {
  registry.register('python:lift_string_compare_prefix', (node, ctx) => {
    const fn = node.childForFieldName('function')
    if (fn?.type !== 'attribute') return null
    if (fn.childForFieldName('attribute')?.text !== 'startswith') return null
    ...
    return createNode('python:string_compare_prefix', {}, kids)
  })
}
