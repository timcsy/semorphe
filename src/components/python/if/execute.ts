...
export function registerExecute(register: (component: string, executor: ComponentExecutor) => void): void {
  register('python:if', async (node, ctx) => {
    ...
    if (ctx.toBool(await ctx.evaluate(node.slots.condition[0]))) {
      await ctx.executeBody(node.slots.body ?? [])
      return
    }
    const conds = node.slots.elif_condition ?? []
    const bodies = node.slots.elif_body ?? []
    for (let i = 0; i < conds.length; i++) {
      if (ctx.toBool(await ctx.evaluate(conds[i]))) {
        ...
        await ctx.executeBody(
          !b ? [] : b.componentId === '_compound' ? (b.slots.body ?? []) : [b],
        )
        return
      }
    }
    await ctx.executeBody(node.slots.else_body ?? [])
  })
}
