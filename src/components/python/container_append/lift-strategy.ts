/** `python:container_append` 的 **lift** 路——認 `x.append(...)`。 */
...
export function registerLiftStrategy(registry: LiftStrategyRegistry): void {
  registry.register('python:lift_container_append', (node, ctx) => {
    const fn = node.childForFieldName('function')
    if (fn?.type !== 'attribute') return null
    if (fn.childForFieldName('attribute')?.text !== 'append') return null
    ...
    return createNode('python:container_append', {}, { obj: [obj], value: [value] })
  })
}
