/** `python:string_replace` 的 **lift** 路——認 `x.replace(...)`。 */
...
export function registerLiftStrategy(registry: LiftStrategyRegistry): void {
  registry.register('python:lift_string_replace', (node, ctx) => {
    const fn = node.childForFieldName('function')
    if (fn?.type !== 'attribute') return null
    if (fn.childForFieldName('attribute')?.text !== 'replace') return null
    ...
    return createNode('python:string_replace', {}, kids)
  })
}
