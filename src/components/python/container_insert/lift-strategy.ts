/**
 * `python:container_insert` 的 **lift** 路——認 `x.insert(…)`。
 *
...
export function registerLiftStrategy(registry: LiftStrategyRegistry): void {
  registry.register('python:lift_container_insert', (node, ctx) => {
    const fn = node.childForFieldName('function')
    if (fn?.type !== 'attribute') return null
    if (fn.childForFieldName('attribute')?.text !== 'insert') return null
    ...
    return createNode('python:container_insert', {}, kids)
  })
}
