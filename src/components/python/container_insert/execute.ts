/**
 * `python:container_insert` 的 **execute** 路——走內建表那一份。
 *
 * 🔴 **不自己算，也不自己挑**：先問使用者定義的類別方法，再回內建表。
 * `insert` 的規則已經寫在 `languages/python/builtins.ts`，
 * 而使用者手寫的 `x.insert(...)` 走的也是那一份。**兩份會先後錯。**
 */
