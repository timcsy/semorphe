/**
 * `python:container_find_position` 的 **lift** 路——認 `x.index(…)`。
 *
...
export function registerLiftStrategy(registry: LiftStrategyRegistry): void {
  registry.register('python:lift_container_find_position', (node, ctx) => {
    const fn = node.childForFieldName('function')
    if (fn?.type !== 'attribute') return null
    if (fn.childForFieldName('attribute')?.text !== 'index') return null
    ...
    return createNode('python:container_find_position', {}, kids)
  })
}
