/**
 * `python:string_compare_suffix` 的 **lift** 路——認 `x.endswith(…)`。
 *
...
export function registerLiftStrategy(registry: LiftStrategyRegistry): void {
  registry.register('python:lift_string_compare_suffix', (node, ctx) => {
    const fn = node.childForFieldName('function')
    if (fn?.type !== 'attribute') return null
    if (fn.childForFieldName('attribute')?.text !== 'endswith') return null
    ...
    return createNode('python:string_compare_suffix', {}, kids)
  })
}
