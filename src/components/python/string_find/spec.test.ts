...
const CODE = "s = \"Hello\"\nprint(s.find(\"l\"), s.find(\"z\"))\n"

describe('python:string_find', () => {
  it('lift：拿得到自己的身分，而不是掉進通用桶', async () => {
    const ids = componentIdsOf(await liftPython(CODE))
    expect(ids).toContain('python:string_find')          // ← 正向錨點
  ...
  it('★ 邊界：別的呼叫不得被認走', async () => {
    ...
    expect(ids).not.toContain('python:string_find')
  })
})
