/**
 * `python:string_find` 的 **lift** 路——認 `x.find(…)`。
 *
...
export function registerLiftStrategy(registry: LiftStrategyRegistry): void {
  registry.register('python:lift_string_find', (node, ctx) => {
    const fn = node.childForFieldName('function')
    if (fn?.type !== 'attribute') return null
    if (fn.childForFieldName('attribute')?.text !== 'find') return null
    ...
    return createNode('python:string_find', {}, kids)
  })
}
