/**
 * `python:container_apply` 的 **lift** 路——認 `map(…)`。
 *
...
export function registerLiftStrategy(registry: LiftStrategyRegistry): void {
  registry.register('python:lift_container_apply', (node, ctx) => {
    if (node.childForFieldName('function')?.text !== 'map') return null
    ...
    if (!a1) return null
    kids['obj'] = [a1]
    return createNode('python:container_apply', {}, kids)
  })
}
