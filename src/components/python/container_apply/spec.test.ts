...
const CODE = "print(list(map(str, [1, 2])))\n"

describe('python:container_apply', () => {
  it('lift：拿得到自己的身分，而不是掉進通用桶', async () => {
    const ids = componentIdsOf(await liftPython(CODE))
    expect(ids).toContain('python:container_apply')          // ← 正向錨點
  ...
  it('★ 邊界：別的呼叫不得被認走', async () => {
    ...
    expect(ids).not.toContain('python:container_apply')
  })
})
