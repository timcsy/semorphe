/**
 * `python:math_divmod` 的 **execute** 路——走內建表那一份。
 *
 * 🔴 **不自己算**：`divmod` 的規則已經寫在 `languages/python/builtins.ts`，
 * 而使用者手寫的 `divmod(...)` 走的也是那一份。**兩份會先後錯。**
 *
 * ## 🔴 使用者自己 `def divmod(…)` 時要讓路
 *
...
export function registerExecute(register: (component: string, executor: ComponentExecutor) => void): void {
  register('python:math_divmod', async (node, ctx) => {
    ...
    if (userDefined) return callWith(userDefined, args, ctx, 'divmod')
    return PYTHON_BUILTIN_FUNCTIONS['divmod'](args, withCall(ctx))
  })
}
