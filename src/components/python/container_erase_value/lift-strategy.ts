/**
 * `python:container_erase_value` 的 **lift** 路——認 `x.remove(…)`。
 *
...
export function registerLiftStrategy(registry: LiftStrategyRegistry): void {
  registry.register('python:lift_container_erase_value', (node, ctx) => {
    const fn = node.childForFieldName('function')
    if (fn?.type !== 'attribute') return null
    if (fn.childForFieldName('attribute')?.text !== 'remove') return null
    ...
    return createNode('python:container_erase_value', {}, kids)
  })
}
