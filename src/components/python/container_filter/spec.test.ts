...
const CODE = "print(list(filter(lambda x: x > 1, [1, 2])))\n"

describe('python:container_filter', () => {
  it('lift：拿得到自己的身分，而不是掉進通用桶', async () => {
    const ids = componentIdsOf(await liftPython(CODE))
    expect(ids).toContain('python:container_filter')          // ← 正向錨點
  ...
  it('execute：與真的 Python 同一個答案', async () => {
    expect(await runPython(CODE)).toContain("[2]")
  })
  ...
  it('★ 邊界：別的呼叫不得被認走', async () => {
    ...
    expect(ids).not.toContain('python:container_filter')
  })
})
