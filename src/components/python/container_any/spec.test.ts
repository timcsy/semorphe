...
const CODE = "print(any([False]), any(x > 0 for x in [1]))\n"

describe('python:container_any', () => {
  it('lift：拿得到自己的身分，而不是掉進通用桶', async () => {
    const ids = componentIdsOf(await liftPython(CODE))
    expect(ids).toContain('python:container_any')          // ← 正向錨點
  ...
  it('execute：與真的 Python 同一個答案', async () => {
    expect(await runPython(CODE)).toContain("False True")
  })
  ...
  it('★ 邊界：別的呼叫不得被認走', async () => {
    ...
    expect(ids).not.toContain('python:container_any')
  })
})
