/**
 * `python:container_any` 的 **lift** 路——認 `any(…)`。
 *
...
export function registerLiftStrategy(registry: LiftStrategyRegistry): void {
  registry.register('python:lift_container_any', (node, ctx) => {
    if (node.childForFieldName('function')?.text !== 'any') return null
    ...
    if (!a0) return null
    kids['obj'] = [a0]
    return createNode('python:container_any', {}, kids)
  })
}
