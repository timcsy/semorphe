...
const CODE = "print(\"7\".zfill(3), \"a\".ljust(3, \"-\"), \"b\".rjust(3))\n"

describe('python:string_fill', () => {
  it('lift：拿得到自己的身分，而不是掉進通用桶', async () => {
    const ids = componentIdsOf(await liftPython(CODE))
    expect(ids).toContain('python:string_fill')          // ← 正向錨點
  ...
  it('execute：與真的 Python 同一個答案', async () => {
    expect(await runPython(CODE)).toContain("007 a--   b")
  })
  ...
  it('★ 邊界：別的呼叫不得被認走', async () => {
    ...
    expect(ids).not.toContain('python:string_fill')
  })
})
