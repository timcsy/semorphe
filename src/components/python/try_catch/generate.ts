...
export function registerGenerate(g: Map<string, NodeGenerator>): void {
  g.set('python:try_catch', (node, ctx) => {
    ...
    return tryHead + block(node.slots.body) + handlers
      + tail('else', node.slots.orelse) + tail('finally', node.slots.ensure)
  })
}
