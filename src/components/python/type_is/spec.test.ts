...
const CODE = "print(isinstance(3, int), isinstance(\"a\", int))\n"

describe('python:type_is', () => {
  it('lift：拿得到自己的身分，而不是掉進通用桶', async () => {
    const ids = componentIdsOf(await liftPython(CODE))
    expect(ids).toContain('python:type_is')          // ← 正向錨點
  ...
  it('execute：與真的 Python 同一個答案', async () => {
    expect(await runPython(CODE)).toContain("True False")
  })
  ...
  it('★ 邊界：別的呼叫不得被認走', async () => {
    ...
    expect(ids).not.toContain('python:type_is')
  })
})
