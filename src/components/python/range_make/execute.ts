/**
 * `python:range_make` 的 **execute** 路——走內建表那一份。
 *
 * 🔴 **不自己算**：`range` 的規則（吃序列還是多引數、空的時候怎麼辦）
 * 已經寫在 `languages/python/builtins.ts`，而使用者手寫的 `range(...)`
 * 走的也是那一份。**兩份會先後錯。**
 *
 * ## 🔴 使用者自己 `def range(x)` 時要讓路
 *
 * 判別在 **lift 期**（這顆元件認走了那個名字），而「有沒有被蓋掉」是
 * **執行期**才知道的事——所以讓路發生在這裡，不是在 lift。
 *
 * ⚠️ 不讓路的症狀：`def range(x): return 99` 之後 `range(a)` 仍然算內建的
 * ——**而積木、來回轉換、型別檢查全部正常**。
 *
...
export function registerExecute(register: (component: string, executor: ComponentExecutor) => void): void {
  register('python:range_make', async (node, ctx) => {
    ...
    return PYTHON_BUILTIN_FUNCTIONS['range'](args, ctx)
  })
}
