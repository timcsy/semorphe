/**
 * `cpp:eeprom_write` 的 **lift** 路——**一個看得到 `obj` 的分支**。
 *
...
export function registerLift(): void {
  registerMethodBranch('cpp/eeprom_write', (obj, method, argChildren, ctx): SemanticNode | null => {
    if (obj !== 'EEPROM' || method !== 'write') return null
    if (argChildren.length !== 2) return null
    const addressNode = argChildren[0] ? ctx.lift(argChildren[0]) : null
    const valueNode = argChildren[1] ? ctx.lift(argChildren[1]) : null
    return createNode('cpp:eeprom_write', { obj }, {
