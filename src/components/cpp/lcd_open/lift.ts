/**
 * `cpp:lcd_open` 的 **lift** 路——**「`LiquidCrystal` 上的 `.begin()` 是我」**
 *
...
export function registerLift(): void {
  registerMethodBranch('cpp/lcd_open', (obj, method, argChildren, ctx): SemanticNode | null => {
    if (method !== 'begin') return null
    if (argChildren.length !== 2) return null
    if (!obj || ctx.data.getType(obj) !== 'lcd') return null
    const colsNode = argChildren[0] ? ctx.lift(argChildren[0]) : null
    const rowsNode = argChildren[1] ? ctx.lift(argChildren[1]) : null
    return createNode('cpp:lcd_open', { obj }, {
      cols: colsNode ? [colsNode] : [],
      rows: rowsNode ? [rowsNode] : [],
    })
  })
}
