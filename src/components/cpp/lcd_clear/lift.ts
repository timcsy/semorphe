/**
 * `cpp:lcd_clear` 的 **lift** 路——**「`LiquidCrystal` 上的 `.clear()` 是我」**
 *
...
export function registerLift(): void {
  registerMethodBranch('cpp/lcd_clear', (obj, method, argChildren, ctx): SemanticNode | null => {
    if (method !== 'clear') return null
    if (argChildren.length > 0) return null
    if (!obj || ctx.data.getType(obj) !== 'lcd') return null
    return createNode('cpp:lcd_clear', { obj }, {
    })
  })
}
