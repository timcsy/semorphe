/**
 * `cpp:servo_declare` 的 **execute** 路——**把這個名字登記成一個物件**。
 *
 * 🔴 **不能 `skipPaths`**：後面的 `myServo.someMethod()` 要查得到它，
...
export function registerExecute(register: (component: string, executor: ComponentExecutor) => void): void {
  register('cpp:servo_declare', async (node, ctx) => {
    const name = String(node.properties.name ?? 'myServo')
    if (!name) throw new Error('硬體物件的宣告少了名字')
    // 引數要**求值過**——`Servo d(DHTPIN, DHTTYPE)` 裡的名字不存在的話要出聲。
    for (const a of node.slots.initializer ?? []) await ctx.evaluate(a)
