/**
 * `cpp:lcd_at` 的 **lift** 路——**「`LiquidCrystal` 上的 `.setCursor()` 是我」**
 *
...
export function registerLift(): void {
  registerMethodBranch('cpp/lcd_at', (obj, method, argChildren, ctx): SemanticNode | null => {
    if (method !== 'setCursor') return null
    if (argChildren.length !== 2) return null
    if (!obj || ctx.data.getType(obj) !== 'lcd') return null
    const colNode = argChildren[0] ? ctx.lift(argChildren[0]) : null
    const rowNode = argChildren[1] ? ctx.lift(argChildren[1]) : null
    return createNode('cpp:lcd_at', { obj }, {
      col: colNode ? [colNode] : [],
      row: rowNode ? [rowNode] : [],
    })
  })
