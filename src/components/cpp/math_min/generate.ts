...
export function registerGenerate(g: Map<string, NodeGenerator>): void {
  g.set('cpp:math_min', (node, ctx) => {
      ...
      //
      // `min({a, b, c})` 是**合法的 C++**（大括號那個多載），而競賽程式常寫它。
      // 補上去的第二個引數讓它**編不過**：
      //
      // ```
      // 原文   min({ans, f(l,mid), f(mid+1,r)})
      // 產出   min({ans, f(l,mid), f(mid+1,r)}, 0)     🔴 拿列表跟 int 比
      // ```
      //
