/**
 * `cpp:servo_write` 的 **lift** 路——**「`Servo` 上的 `.write()` 是我」**
 *
...
export function registerLift(): void {
  registerMethodBranch('cpp/servo_write', (obj, method, argChildren, ctx): SemanticNode | null => {
    if (method !== 'write') return null
    if (argChildren.length !== 1) return null
    if (!obj || ctx.data.getType(obj) !== 'servo') return null
    const angleNode = argChildren[0] ? ctx.lift(argChildren[0]) : null
    return createNode('cpp:servo_write', { obj }, {
      angle: angleNode ? [angleNode] : [],
    })
  })
}
