/**
 * `cpp:range_remove` 的 **lift** 路——**一個帶真邏輯的分支**（引數個數是判別的一部分）。
 */
...
const NAMES = new Set(['remove', 'std::remove'])

export function registerLift(): void {
  registerCallBranch('cpp/range_remove', (funcName, argChildren, ctx, _argsNode): SemanticNode | null => {
    ...
    return createNode('cpp:range_remove', {}, { ...ends, value: [value] })
  })
}
