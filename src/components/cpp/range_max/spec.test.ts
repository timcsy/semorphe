/**
 * `cpp:range_max` 的自證測——每一條負向前面先釘一個正向。
 */
...
describe('膠囊自證：cpp:range_max', () => {
  it('★ lift：認得，且不落進殘差', () => {
    const ids = collect(lift(`vector<int> v={1,9,3}; cout << *max_element(v.begin(), v.end());`))
    expect(ids).toContain('cpp:range_max')          // ← 正向錨點：先證明量到了東西
    expect(ids).not.toContain('raw_code')
    expect(ids).not.toContain('unresolved')
  })

  it('★ generate：產回原樣', () => {
    const code = generateCode(lift(`vector<int> v={1,9,3}; cout << *max_element(v.begin(), v.end());`), 'cpp', apcs as unknown as StylePreset)
    expect(code).toContain('max_element(v.begin(), v.end())')
  })

  it('★ execute', async () => {
    expect(await run(`vector<int> v={1,9,3}; cout << *max_element(v.begin(), v.end());`)).toBe('9')
  })

  it('★ 回傳的是位置不是值', async () => {
    expect(await run(`vector<int> v={1,9,3}; cout << (max_element(v.begin(),v.end())-v.begin());`)).toBe('1')
  })
})
