...
describe('膠囊自證：cpp:string_find_last_not_of', () => {
  it('★ lift：身分由膠囊登錄，而且是字面字串', () => {
    expect(identities(program)).toContain('cpp:string_find_last_not_of')
  })

  it('★ lift 不亂報：沒有這個方法的程式不該產出它', () => {
    const other = identities('int main() { int x = 1; }')
    expect(other).toContain('cpp:var_declare')          // ← 先證明量到了東西
    expect(other).not.toContain('cpp:string_find_last_not_of')
  })
  ...
  it('★ generate：產回 s.find_last_not_of(...)', () => {
    ...
    expect(generateCode(tree, 'cpp', apcs as unknown as StylePreset)).toContain('s.find_last_not_of(')
  })

  it('★ execute："hi  " 去空白的位置是 1', async () => {
    const tree = createTestLifter().lift(parser.parse(program)!.rootNode as never) as SemanticNode
    expect(JSON.stringify(tree)).toContain('cpp:string_find_last_not_of')  // ← 先證明它進了樹
