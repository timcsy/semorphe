...
describe('cpp:define', () => {
  ...
  it('🔴 反向：【運算式位置】的函式巨集刻意不支援，而且必須繼續報錯', async () => {
    // ⚠️ 這一支守的是墓碑的邊界。它變綠的那天，表示有人開始在語義層
    // 求值巨集的運算式——那就是在重建 preprocessor。
    //
    // 🔴 **2026-09-20：這一支的【範圍】縮小了，而那是刻意的。**
    // 原本的名字是「函式巨集刻意不支援」，而那句話現在只對**運算式位置**成立：
    // 展開成一個【語句前綴】的函式巨集（`rep(i,n) for(...)`）做了
    //（見 `languages/cpp/lang/macro-expand.ts`），理由是那一族的展開
    // **有唯一讀法**、而且語料 218 支裡 4 支在用。
    // 而 `SQR(3)` 嵌在 `cout << …` 裡面，它要的是**求值**不是重解一段語句
    // ——那仍然在墓碑那一側。
