/**
 * `cpp:servo_read` 的 **lift** 路——**「`Servo` 上的 `.read()` 是我」**
 *
...
export function registerLift(): void {
  registerMethodBranch('cpp/servo_read', (obj, method, argChildren, ctx): SemanticNode | null => {
    if (method !== 'read') return null
    if (argChildren.length > 0) return null
    if (!obj || ctx.data.getType(obj) !== 'servo') return null
    return createNode('cpp:servo_read', { obj }, {
    })
  })
}
