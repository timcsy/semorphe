...
export class ExecutionController {
  ...
  private async handleRun(): Promise<void> {
    ...
    const tree = this.panels.blocklyPanel?.extractSemanticTree()
    if (!tree) return
    ...
    this.interpreter.setWaitingCallback((blockId) => {
      ...
      if (blockId && this.panels.blocklyPanel?.getWorkspace()) {
        this.panels.blocklyPanel.highlightBlock(blockId, 'execution')
        this.panels.blocklyPanel.getWorkspace()!.centerOnBlock(blockId)
        const mapping = this.panels.syncController?.getMappingForBlock(blockId)
        if (mapping && this.panels.monacoPanel) {
  ...
  private async handleStep(): Promise<void> {
    ...
    this.stepController.onStep(() => {
      ...
      if (step?.blockId) {
        const mapping = this.panels.syncController?.getMappingForBlock(step.blockId)
        if (mapping) {
  ...
  private displayStep(index: number): void {
    ...
    if (step.blockId && this.panels.blocklyPanel?.getWorkspace()) {
      this.panels.blocklyPanel.highlightBlock(step.blockId, 'execution')
      if (autoScroll) {
        this.panels.blocklyPanel.getWorkspace()!.centerOnBlock(step.blockId)
      }
    }
