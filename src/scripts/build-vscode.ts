...
<p align="center">
  <img src="${RAW}/assets/demo-arduino.png" width="900" alt="Arduino IDE 裡：左邊是它自己的編輯器，中間是 Semorphe 積木分頁，右邊是 Semorphe 流程分頁">
</p>

Arduino ＋ 積木的工具幾乎都住在 IDE **外面**（另一個網站、另一個桌面程式、
或一個開獨立視窗的舊外掛），流程是：在那邊拼 → 匯出 \`.ino\` → 切回來 → 燒錄。
...
| **同一個檔** | 那是你正在編輯的 \`.ino\`／\`.cpp\`／\`.py\`——不是匯出的一份拷貝 |
| **原生分頁** | 積木與流程是**編輯器分頁**（拖到哪一欄都行）；主控台與變數在 **panel 那一排**，與終端機並排 |
| **編譯與燒錄照舊** | Arduino IDE 那兩顆 ✓ → 還是它自己的——**我們沒有接管，也不想** |

## 裝好之後按哪裡

1. 開一個 \`.cpp\`、\`.py\`、\`.c\` 或 \`.ino\`（新檔案也行，把語言選成 C++ 或 Python 就會出現）
2. 編輯器右上角的 \`<Σ>\` 圖示 → 開**積木**；
   命令面板（\`Cmd\`/\`Ctrl\`+\`Shift\`+\`P\`）→ **Semorphe: 開啟流程面板** → 開**流程圖**
3. 它們是**編輯器分頁**——拖到右邊、下面、另一個視窗都可以

**主控台與變數**在 panel 那一排（與終端機並排），叫做 \`Semorphe 主控台\` 與
\`Semorphe 變數\`；在 Arduino IDE 裡它們直接在最上面那條功能表列上。

**狀態列**有目標（語言／板子）、課程、章節、鷹架、風格、積木外觀、介面語言與**版面**；
**標題列**是 ▷ 執行 與 ↩↪；**每一格右上角**的下拉決定「這一格顯示哪一層」
——選到別處的就對調。
...
一堂課會**釘住目標、把工具箱收窄到這一課的元件、決定鷹架露多少**。
...
<https://semorphe.com/> — 同一套東西，開瀏覽器就能用，**離線也能跑**。

📱 手機與平板有專為觸控排的版面（四個分頁、選單從下面升起來）——
**沒有電腦的地方一樣學得了程式**。
