...
> 🟢 **2026-09-13：五份宣告在了，而 `mount` 那一半還沒搬。**
>
> ```
> src/panels/⟨每一種⟩/panel.ts   5 份（code · flow · blocks · console · variables）
> loadPanels() 的呼叫者          src/ui/app.ts（組裝點），健檢結果會印出來
> 真的在讀它的消費者              「狀態」那一層的分頁（名字與順序）
> mount 還在組裝點的              5 個，由第一百二十五條護欄棘輪盯著
> ```
>
> 🔴 **在那之前這個目錄裡只有這份 README**，而 `load-panels.ts` 正在 glob 一個
> 零實例的路徑、`loadPanels()` 零個產品呼叫者、下面提到的那支測試不存在。
> 登錄表自己的測試檔頭寫著：
>
> > **一個沒有人宣告的登記處【就是殼】，而它綠得跟真的一樣。**
>
> ⚠️ **`mount` 沒搬是有理由的**，而那個理由是 `app-shell.ts` 自己留下的：
> 那四格的**建構**與各自的相依糾纏在一起（Blockly 要 registry、程式碼走
> `profile.createCodeView`、下方面板要 tabs）。
>
> > **一次抽象如果同時搬走「容器」與「內容」，它壞掉時你分不出是哪一半。**
>
> 🟢 所以先搬「是什麼」（身分／層／順序／名字／在哪個宿主存在），
> **而「怎麼畫」由 `mountedByShell` 說出來、由棘輪數著**
