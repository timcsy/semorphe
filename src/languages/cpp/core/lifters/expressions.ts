...
import { binaryOperatorComponent } from '../../../../core/component/binary-operators'
...
export function registerExpressionLifters(lifter: Lifter): void {
  ...
  lifter.register('binary_expression', (node, ctx) => {
    ...
    const component = binaryOperatorComponent(op)
    if (!component) return null

    return createNode(component, { operator: op }, {
      left: left ? [left] : [],
      right: right ? [right] : [],
    })
  })
  ...
  // ⚠️ `cpp:initializer_list` 自己宣告「我是一層 `{…}`」（在它的膠囊裡）。
  ...
  lifter.register('initializer_list', (node, ctx) => {
    ...
    return buildInitializerList(values)
  })
