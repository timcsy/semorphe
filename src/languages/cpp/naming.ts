...
export const OPERATIONS = [
  ...
  'sync', 'tie',
  // ⚠️ `step` 加入日 2026-09-19——`cpp:pointer_step`（`prev(it)`／`next(it)`）。
  //
  // 「往某個方向走一格」。🔴 **而它不叫 `next`**（`next` 已經在這張表裡，
  // 由 `cpp:random_next` 用著「下一個亂數」那個無方向的語義）：
  // 用 `next` 命名一顆**可以往前也可以往後**的身分，
  // 等於把一個屬性的值寫進身分——那正是這個 repo 一再治的病
  //（「位置不是身分，是形態」／「量什麼不是身分，是參數」）。
  //
  // ⚠️ 也不叫 `move`：`std::move` 在 C++ 裡是所有權轉移，撞名。
  //
  // 🟢 **而主體沒有新增**：第一版取名 `cpp:iter_step`，命名護欄擋下來
  //（`iter` 不是主體，它是 `container_iter` 的操作詞）。改成 `pointer` 之後
  // 只剩一個新詞——而那個主體是**這個 repo 自己的話**：
  // `interpreter/pointer.ts` 的檔頭逐字「**迭代器 ＝ 實體式指標**」。
  //
  // > **護欄擋下一個名字的時候，先問它是不是在指一個更好的名字。**
  'step',
  ...
  /**
   * ⚠️ 兩個加入日 2026-09-18——「刪除-移除」那個慣用法的兩顆
   *（`cpp:range_remove`／`cpp:range_unique`）。
   *
   * 🔴 **`remove` 不與既有的 `erase` 合併，而那正是這兩顆存在的理由**：
   *
   * ```cpp
   ...
   * ```
   *
   * 合併成同一個字的話，上面那一行會變成「erase 一個 erase 的結果」
   * ——而**學生看不懂那句話，正如同他們看不懂為什麼要寫兩次**。
   * > **同義詞合併的判準是語義，不是詞形**（見上面 `attach` 那一條）。
   *
   * ⚠️ `unique` 與 `remove` 做的其實是**同一件事**（把不要的往後擠），
   *    差在判準：一個問「等不等於這個值」，一個問「與前一格一不一樣」。
   *    而那個差別在接點結構上看得見（前者多一個接點），所以是兩顆身分。
   */
  'remove', 'unique',
  /**
   * ⚠️ `flush` 加入日 2026-09-19——`cout << flush`。
   *
   * 🔴 **與 `push`／`write` 都不合併**：它送出的**不是一個值**，
   * 是「把已經交出去的東西真的推出去」——那個動作沒有對象。
   * 而與同族那顆換行的（它做兩件事：換行＋送出）也不是同一顆。
   *
   * ⚠️ 它是那一族的第三個（`sync`／`tie` 2026-08-13 加的，
   * 都是「串流的某個開關」），而**三個都不是同義詞**：
   * 一個是「兩邊保持一致」、一個是「一邊動另一邊跟著動」、這一個是「推出去」。
   *
   * 🟢 而輸出設定那兩顆**一個新詞都沒有用**（`io` ＋ `mode` ＋ `number` 都在表裡）
   * ——那是詞彙表在做它該做的事：它逼出的第一版名字（`stream_*`）
   * 用了三個不在表裡的字，而換成既有詞之後名字反而對得上同族
   *（`cpp:io_sync`／`cpp:io_tie` 早就在那裡）。
   */
  'flush',
] as const
