 ...
 *
 * ⚠️ ~~一個真實的差別：C 風格轉型的 `char` 要回字元，命名轉型的 `char` 回整數
 * （既有行為，**未經檢驗是否正確**——保留，不在搬家的 diff 裡改）。~~
 *
 * ✅ **2026-08-13 檢驗了，而那個差別是錯的**：真編譯器對
 * `static_cast<char>(66)` 也印出 `B`。五顆現在全部 `charIsChar: true`。
 *
 * > **「未經檢驗是否正確」的東西被原樣保留了五個月——而保留它的那句話
 * > 本身就是它可以被檢驗的證據。**
 */
